...
Preserve the test-induced `M1-SRC-001` materialization attempt `fixture-must-not-run` and its repository receipt. Its 26-file manifest passed independent hash verification and is classified only as valid materialization with unintended-test provenance. It establishes no raster readability, pixel usability, baseline, change, or scientific admission and releases no next processing.
