...
Sentinel-1 orbit correction is approved and active at the dependent checkpoint `M2-ORBIT-SENTINEL-CUSTODY`. Review bundle SHA-256 `ee5fbf4933b52be8f97441b78a73559a973bd975efc21b43625f1ceca54e2ff1` and proposal SHA-256 `b17e256068759946be611bf4e7beffe0d3121e9e731b6c42163525eca2cf0292` authorize only four exact S1D `AUX_RESORB` files for their six exact Sentinel source bindings. Fresh preflight passed and empty non-Git custody is initialized; preserve the failed first initialization and its bounded correction. The transfer runner must stop before catalogue or token access until every source bound to the selected orbit is promoted and offline container-verified. The active intake passes the generic intake validator after recorded corrections added the reason that pre-transfer SHA-256 is unavailable and replaced its stale candidate root label with `active`. Do not silently substitute a later `AUX_POEORB` file: precise-orbit selection requires a fresh exact manifest and review. The DEM vertical-datum and terrain-result gates, Sentinel custody, pixel readiness, radar processing, and scientific claims remain separate.
