...
`config/qa/dem-terrain-quality-contract.json` predeclares an independent read-only terrain-quality check. It may read only the four exact promoted DEM tiles and write one exclusive derived QA attempt beneath the approved external data root. It applies no vertical transform and may not publish DEM-derived raster imagery. Preserve any failed attempt and do not change thresholds after metrics are observed.
