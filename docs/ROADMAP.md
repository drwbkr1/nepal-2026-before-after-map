...
**Terrain-quality gate:** Before reading real terrain values, bind the four exact promoted DEM hashes, the two east-west and two north-south native seams, fixed artifact and slope thresholds, and a no-overwrite output. Attempt-001 failed before source open because the declared external root omitted the `custody` segment; it is retained and its path will not be reused. Attempt-002 corrects only that path join and declares `derived/dem-terrain-quality/attempt-002`. ArcGIS must horizontally project the mosaic to EPSG:32645 at 30 metres without a vertical transform, calculate AOI slope and hillshade, export an external APRX/PNG/PDF, and preserve quantitative and visual outcomes. A passing terrain-only result cannot resolve the independent vertical-datum, ground-accuracy, radar-processing, or scientific gates.
