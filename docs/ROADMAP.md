...
**Status:** Active at `M2-DEM-VERTICAL-DATUM-REVIEW`. All eight exact Sentinel archives are promoted, container-verified, and materialized by identity; the optical route remains terminal `INVALID`/`BLOCK`; and all four exact `AUX_RESORB` files pass durable input-only verification. The owner accepted the exact attempt-003 terrain screen, closing only its human-review gate. The reassessed terrain audit remains `DEFER` on independent accuracy evaluation, pair-specific radar-input fitness, and vertical and elevation uncertainty. Orbit application, radar pixels, the baseline, change analysis, and scientific fitness remain unresolved.
