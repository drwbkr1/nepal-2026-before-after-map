...
**Status:** Active at orbit recovery-002 implementation publication. All eight exact Sentinel archives are promoted, container-verified, and materialized by identity; both earlier incomplete `M1-SRC-004` partials and the stopped first continuation remain preserved. Optical real-001 remains terminal `INVALID`, and recovery-001 remains terminal `BLOCK`. The corrected orbit recovery review now has one exact attested approval. Its bounded worker passes 12 focused tests and the full 395-test repository suite locally, while public CI, activation, and the final no-payload preflight remain pending. The approval covers only one possible byte-zero recovery of `M2-ORB-001`. The four-tile DEM amendment remains at `M2-DEM-VERTICAL-DATUM-REVIEW`, while the terrain screen has a separate `M2-DEM-TERRAIN-RESULT-REVIEW`. Vertical-datum fitness, independent elevation accuracy, radar pixel fitness, and scientific fitness remain unresolved.
