...
**Status:** Active with one Sentinel checkpoint, one dependent orbit checkpoint, and two independent DEM review checkpoints. The exact eight-product Sentinel plan remains at `M2-AUTHENTICATION-REFERENCE`: its live source and storage preflight passed and empty custody is initialized, but no product bytes have been transferred. The exact four-file `AUX_RESORB` amendment is approved and active at `M2-ORBIT-SENTINEL-CUSTODY`; its fresh source and rights preflight passed and empty custody is initialized, but its runner stops before catalogue or token access until the bound Sentinel sources are promoted and offline-verified. The approved four-tile DEM amendment remains at `M2-DEM-VERTICAL-DATUM-REVIEW`, while the completed terrain screen has a separate `M2-DEM-TERRAIN-RESULT-REVIEW`. All four exact DEM tiles are promoted and have passed ArcGIS structural, valid-AOI-coverage, fixed terrain-artifact, seam, slope, projection, manifest, and exported-map checks. Vertical-datum fitness, independent elevation accuracy, pair-specific radar fitness, and scientific fitness remain unresolved.
...
**Activated Sentinel-1 orbit amendment:** Official CDSE metadata produced one deterministic full-coverage S1D `AUX_RESORB` selection for each of the four unique radar acquisition windows. The four files total 2,539,715 bytes and bind exactly to the six approved Sentinel-1 source IDs. The owner approved exact bundle `ee5fbf4933b52be8f97441b78a73559a973bd975efc21b43625f1ceca54e2ff1` and proposal `b17e256068759946be611bf4e7beffe0d3121e9e731b6c42163525eca2cf0292`; the response was locked, reconciled, and activated. Fresh preflight found all four identities online and unchanged and matched the reviewed rights bytes. Custody attempt-001 failed after creating seven empty directories because an event parent was omitted; that failure is retained. A predeclared attempt-002 verified the exact empty partial inventory and completed the approved directory structure. Twenty-nine focused tests and a guard probe validate the transfer and EOF-verification controls, but zero orbit payloads exist and transfer remains blocked on the six Sentinel custody prerequisites. No covering `AUX_POEORB` candidate was available at assessment time, and later precise substitution remains separately gated.
