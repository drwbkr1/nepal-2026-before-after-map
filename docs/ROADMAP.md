...
**Status:** Active at `M2-DEM-EGM2008-COMPONENT-INSTALL`. All eight exact Sentinel archives are promoted, container-verified, and materialized by identity; the optical route remains terminal `INVALID`/`BLOCK`; and all four exact `AUX_RESORB` files pass durable input-only verification. The owner accepted the exact attempt-003 terrain screen and selected the exact EGM2008 one-minute preconversion method. The matching optional ArcGIS component remains absent. The reassessed terrain audit remains `DEFER` on independent accuracy evaluation, pair-specific radar-input fitness, and vertical and elevation uncertainty. DEM conversion, orbit application, radar pixels, the baseline, change analysis, and scientific fitness remain unresolved.
...
**Vertical-datum route:** The owner approved bundle SHA-256 `9b40e81df766ea866c5bff51cdbc4d83e7e7da6a554fb1709fc553d8221bebbc` and selected ArcGIS's optional EGM2008 one-minute preconversion method: convert verified DEM copies from EPSG:3855 orthometric height to WGS 84 ellipsoidal height, then use `NONE` only for those verified derivatives. The local runtime currently has only the built-in EGM96 grid, so the exact route cannot run. My Esri sign-in, license acceptance, component download and installation, and UAC remain owner-controlled. After the owner installs `world1x1_vert`, a fresh read-only capability inspection must verify the exact grid, transformation, direction, and AOI coverage before conversion controls are prepared.
