...
**Status:** Active at `M2-DEM-VERTICAL-DATUM-ALTERNATE-METHOD-001-REVIEW`. All eight exact Sentinel archives are promoted, container-verified, and materialized by identity; the optical route remains terminal `INVALID`/`BLOCK`; and all four exact `AUX_RESORB` files pass durable input-only verification. The owner accepted the exact attempt-003 terrain screen and selected the exact EGM2008 one-minute preconversion method, but Purdue cannot supply the matching optional ArcGIS component and the fresh reinspection still finds it absent. A zero-decision review now proposes an official PROJ-data 2.5-minute replacement. The reassessed terrain audit remains `DEFER` on independent accuracy evaluation, pair-specific radar-input fitness, and vertical and elevation uncertainty. DEM conversion, orbit application, radar pixels, the baseline, change analysis, and scientific fitness remain unresolved.
...
**Approved post-optical route split:** Analysis SHA-256 `19672e1d646b5ee6131f57c99710c2483b9deb17743f833dd662edaa8afd128e` identified the aggregate dependency conflict. The owner approved bundle SHA-256 `5a5bd80f724841f9558ad5ff966ed0d49222419f7310b345492172e4639421ad` and proposal SHA-256 `ae2ddfa153a86b7acf7f8ec500690713d5ced9a8ddd58f5655d831e1eb282c77`. The amendment preserves both optical terminal outcomes, adds route-specific radar custody/materialization/header readiness, classifies the old orbit packet as stale evidence, and prepares corrected orbit-recovery bundle SHA-256 `6d43342b6bda2740667fa6e924a52f15313d8827cfb62563ea107bc483e87fa5` with proposal SHA-256 `d30208c07deb66ef2c7487f8c901abd4fb5ff04aa56766bca8066d4c8d4f0db8`. The corrected packet remains blank and releases no orbit, DEM, pixel, baseline, or change action.
...
**Vertical-datum route:** The owner approved bundle SHA-256 `9b40e81df766ea866c5bff51cdbc4d83e7e7da6a554fb1709fc553d8221bebbc` and selected ArcGIS's optional EGM2008 one-minute preconversion method: convert verified DEM copies from EPSG:3855 orthometric height to WGS 84 ellipsoidal height, then use `NONE` only for those verified derivatives. The local runtime has only the built-in EGM96 grid, Purdue reports that it cannot supply the optional component, and a fresh reinspection confirms the exact Esri route remains unavailable. Review bundle SHA-256 `caa27cad02aa78caeb38c511ea3cae7ebb637833bd4d54881b8a557a38eb1702` now proposes exact `us_nga_egm08_25.tif` from the official PROJ CDN, with byte length 80,585,622 and SHA-256 `4191d471eefebf24091b56dbc604353cb3b8cf8cc70e448bb9ae56a272bef17a`, using the existing ArcGIS-bundled GDAL 3.12.2e and PROJ 9.8.1 runtime with networking disabled. This is a 2.5-minute substitution for the previously selected one-minute grid and requires an explicit owner decision before acquisition, implementation, or conversion.

**Terrain-quality gate:** The four exact promoted DEM hashes, two east-west and two north-south seams, fixed artifact and slope thresholds, and no-overwrite outputs were bound before observation. Attempt-001 failed before source open because its root omitted the `custody` segment. Attempt-002 created outputs but failed before a stable manifest while opening a transient ArcGIS lock; its metrics were not persisted. Both failures and paths remain preserved. Attempt-003 excluded only `.lock` files, hashed all 189 stable files, reverified unchanged source custody, horizontally projected the mosaic to EPSG:32645 at 30 metres without a vertical transform, calculated AOI slope and hillshade, and exported the external APRX/PNG/PDF. All quantitative and visual criteria passed. The exact owner approval of bundle SHA-256 `834ad354fc134b2017afdd3b238c1a6271276e8b1a95776e434180c7283a26d5` now makes only `human-review` pass. The readiness result remains `defer`; vertical datum, independent elevation accuracy, pair-specific radar processing, and scientific fitness remain unresolved.

**Activated Sentinel-1 orbit amendment:** Official CDSE metadata produced one deterministic full-coverage S1D `AUX_RESORB` selection for each of the four unique radar acquisition windows. The four files total 2,539,715 bytes and bind exactly to the six approved Sentinel-1 source IDs. A stale production-wrapper test later reached a rejected `M2-ORB-001` request using a tracked nonsecret literal. It received zero payload bytes, created no payload file, and remains terminal evidence. The old recovery bundle `df5aa9d0d03f8ee30a5cd74b91f74a88c83a525e762c22b0bd2b6773ccb5bc6b` is preserved but not actionable. The current runner requires completed route-specific radar readiness and completed recovery-002 before catalogue or token access; current safe probes stop at `orbit_recovery_002_unit_not_complete`. The corrected review authorizes nothing until a separate completed owner response is locked and reconciled. No covering `AUX_POEORB` candidate was available at assessment time, and later precise substitution remains separately gated.
