...
**Prepared radar input gate:** A fixed offline contract now binds the three materialized pre-event radar receipts before any real SAFE header inspection. Fourteen portable tests and a synthetic ArcGIS run pass exact inventory, safe XML, acquisition-bracketing embedded vectors, VV/VH metadata agreement, U16 measurement headers, and deliberate mismatch refusal. Real metadata and TIFF headers remain unread until the published gate passes CI. A later real pass would remain partial and cannot replace the missing post-event scenes or release baseline processing.
