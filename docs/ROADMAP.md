...
**Status:** Active at `M2-ORBIT-RECOVERY-002-REVIEW`. All eight exact Sentinel archives are promoted, container-verified, and materialized by identity; both earlier incomplete `M1-SRC-004` partials and the stopped first continuation remain preserved. Optical real-001 remains terminal `INVALID`, and recovery-001 remains terminal `BLOCK`. The approved radar-first amendment keeps aggregate `M2-VERIFY` deferred while a separate six-source radar unit records custody, materialization, and header readiness only. The old aggregate-dependent orbit packet remains stale evidence. The corrected orbit review contains zero decisions and concerns only one possible future byte-zero recovery of `M2-ORB-001`; it releases no current access or processing. The four-tile DEM amendment remains at `M2-DEM-VERTICAL-DATUM-REVIEW`, while the terrain screen has a separate `M2-DEM-TERRAIN-RESULT-REVIEW`. Vertical-datum fitness, independent elevation accuracy, radar pixel fitness, and scientific fitness remain unresolved.
