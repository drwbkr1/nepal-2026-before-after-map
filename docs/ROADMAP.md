...
**Terrain-quality gate:** Before reading real terrain values, bind the four exact promoted DEM hashes, the two east-west and two north-south native seams, fixed artifact and slope thresholds, and a no-overwrite output. Attempt-001 failed before source open because its root omitted the `custody` segment. Attempt-002 corrected the path and created its outputs but failed before a stable manifest while opening a transient ArcGIS lock; its metrics were not persisted. Both failures and paths remain preserved. Attempt-003 excludes only `.lock` files from the stable inventory and declares `derived/dem-terrain-quality/attempt-003`. ArcGIS must horizontally project the mosaic to EPSG:32645 at 30 metres without a vertical transform, calculate AOI slope and hillshade, export an external APRX/PNG/PDF, and preserve quantitative and visual outcomes. A passing terrain-only result cannot resolve the independent vertical-datum, ground-accuracy, radar-processing, or scientific gates.
