...
The exact three-source Sentinel-1 input-readiness gate is now prepared but has not inspected a real SAFE. It binds the three repository receipt hashes and external manifest hashes, requires nine exact SAFE members per source, validates S1D/IW/GRD annotation identity, VV/VH metadata agreement, and finite ordered embedded vectors that bracket each acquisition, and permits ArcGIS to open only the six VV/VH measurement TIFF headers after every non-ArcGIS check passes. Fourteen portable tests pass. ArcGIS Pro 3.7.1 Advanced opened six synthetic U16 TIFFs; all aligned synthetic sources passed header-readiness only and a deliberate VH-width mismatch blocked. Six earlier prepublication attempts remain visible, including one failed run and its five tiny files after an ArcGIS `datetime` name collision. The sixth passing attempt was superseded after publication review found that a pre-ArcGIS inventory block could evaluate an unavailable ArcPy license fallback while constructing its receipt; the corrected runner now records that blocked path without importing ArcPy. The final synthetic receipt passes. No external custody, real product metadata, real raster header, or real pixel was accessed by these control validations, and no baseline action is released.
