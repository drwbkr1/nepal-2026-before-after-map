...
The current checkpoint is `M2-ACQUISITION-REVIEW`. Do not delete or resume the partial, retry `M1-SRC-004`, or continue another Sentinel transfer until the exact retained-failure recovery is reviewed. No secret value is recorded. Do not place a token, password, cookie, refresh value, or authorization header in chat, Git, a filename, a receipt, or captured command output.
