...
ArcGIS Pro must open every selected JP2 as one band in EPSG:32645. B02, B03, B04, and B08 must be unsigned reflectance DNs on a 10 m grid; B11, B12, and SCL must be on a 20 m grid. Every raster's dimensions multiplied by its cell size must reproduce its reported extent. The quantitative band and SCL extents must agree within 0.001 m inside each product, and matching roles must have the same dimensions, cell sizes, pixel types, and extents across the before/after pair.

The quality-classification JP2 must be readable, single-band, projected to EPSG:32645, and nonempty. Its resolution is recorded but is not silently forced to the quantitative grid at this gate.
...
ArcGIS Pro 3.7.1 Advanced opened sixteen deterministic JPEG2000 fixtures generated through its bundled GDAL 3.12.2 `JP2OpenJPEG` driver. The fixture contained matching 10 m and 20 m EPSG:32645 grids, baseline 05.12 metadata, one quantification value, and all six required band offsets. The aligned pair passed header readiness and a deliberate 10 m shift of the complete after grid produced fourteen cross-pair extent errors and `block`.

Two earlier attempts to create JP2 fixtures directly with ArcGIS `CopyRaster` failed with `No raster store is configurated.` They remain recorded. Five prepublication passing receipts were superseded in sequence: the first omitted detailed headers, the second preceded runner hardening for retained metadata and header failures, the third preceded the dimension-to-extent consistency rule, the fourth followed a failed portable fixture expectation in the same chained invocation, and the fifth preceded explicit attempted-versus-complete header-open activity fields. No attempt used real source data.
...
& "C:\Program Files\ArcGIS\Pro\bin\Python\Scripts\propy.bat" `
  ...
  --output-root scratch\optical-input-readiness-arcgis-007 `
  --receipt-output records\surface-receipts\optical-input-readiness-synthetic-arcgis-007.json `
...
[Esri documents JPEG2000 (`.jp2`) as a supported ArcGIS raster dataset format](https://pro.arcgis.com/en/pro-app/latest/help/data/imagery/supported-raster-dataset-file-formats.htm). The project nevertheless validates the installed runtime directly because format support does not prove the identity, CRS, grid, or readability of these event products.
