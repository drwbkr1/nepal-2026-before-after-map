...
The first package-portability control is deliberately narrower than final delivery. `config/qa/arcgis-package-portability-contract.json` bound the validated metadata-only workspace to one fresh external `.ppkx` attempt, extraction, reopen, and 160 dpi re-export. That exact publication-gated attempt passed with self-contained operational sources, zero scientific rows, unchanged source inventory, and a pixel-identical visually inspected export. A post-run verification mistake created a separate unplanned extraction directory; it is retained and prevents an unqualified process-conformance pass. Clean-machine and final scientific-package verification remain open.

## Predeclared final acceptance

`config/qa/arcgis-final-delivery-contract.json` is the distinct M6 acceptance boundary for the completed scientific package. Unlike the metadata fixture, it cannot pass with zero reviewed observations. It requires all five planned maps, EPSG:32645 scientific vectors and analysis GeoTIFFs, exact before/after source joins, uncertainty and limitations, separate linked interpretation and attribution records, complete QA, visible exclusions, and reconciliation of failed, deferred, and inconclusive routes.

Final acceptance also requires a SHA-256 manifest for the APRX, File Geodatabase, layer files, GeoTIFFs, GeoPackage, PPKX, paired PNG/PDF exports, and delivery README. The package must extract, reopen, and re-export in a clean machine or clean local profile where the original workspace is absent. See [ARCGIS_FINAL_DELIVERY_ACCEPTANCE.md](ARCGIS_FINAL_DELIVERY_ACCEPTANCE.md). A passing final-delivery report can close only M6; M5 scientific review and M7 publication remain separate gates.
