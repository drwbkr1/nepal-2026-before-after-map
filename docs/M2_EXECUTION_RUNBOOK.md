...
`config/qa/radar-input-readiness-contract.json` predeclares the next read-only check for exactly those three SAFE directories. Its production runner requires the fixed materialization-receipt hashes, complete markers, and selected-member hashes before ArcPy import; parses exact S1D/IW/GRD acquisition and embedded-orbit metadata; opens only six VV/VH TIFF headers; and hashes each complete attempt before and after access. Any new sidecar or changed byte blocks. The final synthetic ArcGIS control passes, but the real runner remains unexecuted until this gate is first published and reverified.
