...
The current checkpoint is `M2-ACQUISITION-REVIEW`. An existing owner-controlled DPAPI credential reference generated fresh process-local CDSE access tokens without recording secret values. `M1-SRC-001` through `M1-SRC-003` are promoted and passed offline container verification. `M1-SRC-004` stopped on `transferred_size_mismatch` after 561,593,598 of 1,732,332,897 bytes; its partial and terminal events are retained. Four products remain unattempted. Do not delete or resume the partial, retry the failed source, or continue another Sentinel transfer before exact recovery review. Never place secret values in a command argument, chat message, repository file, receipt, URL, or captured output.

The exact recovery decision point is review bundle SHA-256 `dffa194cc91636a35b5f55af6ece32bb6eb90d77b65ea3d9865413f912d146e7` and proposal SHA-256 `7b8b5e83265b37962f879ca7dad85ab5f5c04ceb28ee0f15fa774a79df7fd013`. Its blank response contains zero human decisions. Do not implement or run recovery from the prepared proposal alone.

A separate DEM amendment was approved on 3 September 2026 through review bundle SHA-256 `caecbdfe69ec1a6c8c39401b63756005820a727cb8f9e7e0084753e2d6afb39e`, proposal SHA-256 `92f48680c0b779398d8bbebd872a60bc3850f008f5c9b68d5bf45a2448abdd69`, and license SHA-256 `9cd37d37ea654bbcaf0a2e059e6a3a5b5f76072824d8dd860ccf274ada8951bd`. Its four exact tiles are now promoted and structurally verified, and its independent checkpoint is `M2-DEM-VERTICAL-DATUM-REVIEW`. It does not change the Sentinel retained-failure review.

The exact four-file Sentinel-1 orbit amendment was approved on 4 September 2026 through review bundle SHA-256 `ee5fbf4933b52be8f97441b78a73559a973bd975efc21b43625f1ceca54e2ff1` and proposal SHA-256 `b17e256068759946be611bf4e7beffe0d3121e9e731b6c42163525eca2cf0292`. Its fresh source-and-rights preflight and corrected empty-custody initialization pass. A test-induced `M2-ORB-001` attempt later failed on a rejected download request with zero payload bytes and no payload file; its events and receipt are retained. The dependent checkpoint is now `M2-ORBIT-ACQUISITION-REVIEW`. No orbit transfer is eligible until the full `M2-VERIFY` unit is complete and the exact retained-failure review is approved and reconciled. Later precise substitution remains separately gated.
...
The first real acquisition sequence produced three passing promotions and one retained failure. Reconciliation record `records/acquisition/sentinel-acquisition-reconciliation-001.json` binds the current intake, all three transfer and container receipts, the failed receipt, external event hashes, and the retained partial hash. Because the failed response did not preserve both verified byte-range support and an unchanged strong remote object identity, resumption is not eligible. Any approved recovery must use a distinct fresh staging path, restart from byte zero, preserve the failed attempt unchanged, and stop again on any mismatch.
...
`contracts/m2-materialization.json` defines the next offline step. `scripts/materialize_m2_product.py` refuses any asset without one exact promoted intake attempt and its matching `pass_container_only` receipt. It re-hashes the archive, rejects unsafe Windows and cross-platform member paths, and extracts into a new append-only external attempt while hashing every file. Partial or failed attempts remain visible. See [M2_SAFE_MATERIALIZATION.md](M2_SAFE_MATERIALIZATION.md). All three currently promoted radar archives have reached this gate: `M1-SRC-001` retains unintended-test provenance, while `M1-SRC-002` and `M1-SRC-003` are planned passing attempts. Their 78 files and 5,183,550,209 extracted bytes match the external manifests, but no raster or pixel gate has passed.

`config/qa/radar-input-readiness-contract.json` predeclared the read-only check for exactly those three SAFE directories. Its production runner required fixed receipt, marker, and selected-member hashes before ArcPy import; parsed exact S1D/IW/GRD acquisition and embedded-orbit metadata; opened only six VV/VH TIFF headers; and hashed each complete attempt before and after access. After publication and successful public CI, the one real invocation blocked because all six annotations use `Detected` instead of the frozen `AMPLITUDE` value. The exact receipt and unchanged external inventory are retained. Do not rerun or adjust this contract; an authoritative label review and separately reviewed amendment are required first.
...
Before any orbit transfer, require the full `M2-VERIFY` unit in `contracts/milestone-002.json` to be `complete`. The runner checks this milestone dependency before public catalogue access, before reading the `CDSE_ACCESS_TOKEN` environment reference, and before writing an event or payload. It also retains the narrower exact scene-to-orbit custody checks. The current guard result is exit 12, `sentinel_verification_unit_not_complete`, with zero orbit payload files.

After the Sentinel recovery succeeds, all eight products complete acquisition and verification, and the exact orbit recovery review is approved and reconciled, recover only the exact failed orbit through the separately implemented recovery identity. Do not invoke the ordinary acquisition command against failed `M2-ORB-001`. After a passing recovery, transfer only one still-authorized orbit at a time:

```powershell
python scripts/acquire_m2_orbit_file.py --source-id M2-ORB-002
```

The runner requires an unchanged exact CDSE identity, provider MD5 and BLAKE3, local SHA-256, exclusive staging, preserved failures, and atomic no-replace promotion. The active intake's unknown pre-transfer SHA-256 fields include the schema-required reason, its root status is `active`, and the generic intake validator passes; the earlier missing-field failure and stale-label finding remain recorded. After promotion, `scripts/verify_m2_orbit_eof.py` must reject external XML entities and require exact mission, file type, validity window, scene binding, and ordered finite state vectors. Do not invoke either route now. Orbit recovery review bundle SHA-256 `df5aa9d0d03f8ee30a5cd74b91f74a88c83a525e762c22b0bd2b6773ccb5bc6b` binds proposal SHA-256 `ce76d633a8104ea5800f51dccd4b1037f930d41b7f08a3de32eed68c6697915a`, contains zero human decisions, and creates no authority by itself.
