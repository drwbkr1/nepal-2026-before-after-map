...
The current checkpoint is `M2-ORBIT-RECOVERY-002-REVIEW`. All eight exact Sentinel archives are promoted, container-verified, and materialized by identity. The six Sentinel-1 sources pass the route-specific custody, materialization, and header-readiness unit without measurement-pixel decoding. Both older incomplete `M1-SRC-004` partials remain retained. Never place secret values in a command argument, chat message, repository file, receipt, URL, or captured output.
