...
The current operational gate is orbit recovery-002 implementation publication. All eight exact Sentinel archives are promoted, container-verified, and materialized by identity. The six Sentinel-1 sources pass the route-specific custody, materialization, and header-readiness unit without measurement-pixel decoding. The owner approved only the exact recovery-only worker, public CI, final no-payload preflight, and one future byte-zero `M2-ORB-001` attempt. Never place secret values in a command argument, chat message, environment variable, repository file, receipt, URL, or captured output.

The earlier Sentinel recovery and continuation decisions are complete historical controls. They do not authorize any orbit request, DEM action, radar pixel read, baseline, or change analysis.
...
Before any ordinary orbit transfer, require both `M2-RADAR-SOURCE-READINESS` and `M2-ORBIT-RECOVERY-002` in `contracts/milestone-002.json` to be `complete`. The runner validates the six-source readiness record and recovery unit before public catalogue access, before reading the token reference, and before writing an event or payload. The current guard result is exit 12, `orbit_recovery_002_unit_not_complete`, with zero orbit payload files.

Only a future completed approval of the corrected review could release a separately implemented, public-CI-gated, one-attempt recovery of exact `M2-ORB-001`. Do not invoke the ordinary acquisition command against failed `M2-ORB-001`. The corrected review does not authorize requests for `M2-ORB-002` through `004`. If later authority and all gates release continuation, the ordinary command remains one source at a time:

```powershell
python scripts/acquire_m2_orbit_file.py --source-id M2-ORB-002
```

The runner requires an unchanged exact CDSE identity, provider MD5 and BLAKE3, local SHA-256, exclusive staging, preserved failures, and atomic no-replace promotion. After promotion, `scripts/verify_m2_orbit_eof.py` must reject external XML entities and require exact mission, file type, validity window, scene binding, and ordered finite state vectors. Do not invoke either route now. The old bundle SHA-256 `df5aa9d0d03f8ee30a5cd74b91f74a88c83a525e762c22b0bd2b6773ccb5bc6b` remains stale, unapproved evidence and cannot release any action.
