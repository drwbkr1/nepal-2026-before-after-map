...
ArcGIS Pro's installed Sentinel-1 terrain-correction tools also expose a required DEM input. On 3 September 2026 the owner approved the exact four-tile Copernicus DEM GLO-30 amendment and accepted the hash-bound Copernicus WorldDEM-30 license. The active DEM intake and offline verification controls cover only those four anonymous HTTPS objects. Activation itself made no network request, created no external DEM path, and requested no DEM payload; the parallel workstream is at `M2-DEM-FRESH-PREFLIGHT`. The original eight-product acquisition remains independently paused at the secure authentication handoff.
