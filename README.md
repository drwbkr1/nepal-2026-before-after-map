...
This goal is **active as of 3 September 2026**. M1 is complete: the search and review AOIs are owner-approved, projected to EPSG:32645, and validated through ArcGIS Pro, and the exact source manifest is approved with eight sources accepted for controlled acquisition and two preserved as deferred context. M2 was activated by an exact owner decision on 3 September 2026. The eight Sentinel products remain at the secret-safe authentication checkpoint with zero recorded transfer attempts and zero payload files; a terms-page guard stop was reconciled and the live eight-product preflight was refreshed without mutation. Separately, four approved Copernicus DEM tiles totaling 170,302,058 bytes were anonymously acquired outside Git and passed exact-byte, ArcGIS structural, and AOI finite-coverage checks. The owner has also approved four exact S1D restituted orbit files for the six radar scenes. Their live metadata and reviewed rights bytes pass fresh preflight, empty non-Git custody is initialized, and the guarded runner is ready, but transfer remains blocked until the bound Sentinel scenes reach promoted, offline-verified custody. No Sentinel product, orbit payload, vertical-datum conversion, orbit correction, radar result, or scientific change has been established.
...
The parallel orbit route is now active at `M2-ORBIT-SENTINEL-CUSTODY`. The exact decision for review bundle SHA-256 `ee5fbf4933b52be8f97441b78a73559a973bd975efc21b43625f1ceca54e2ff1` and proposal SHA-256 `b17e256068759946be611bf4e7beffe0d3121e9e731b6c42163525eca2cf0292` was locked, reconciled, and activated. Fresh preflight found all four approved `AUX_RESORB` identities online and unchanged and matched the reviewed terms and legal-notice bytes. The first empty-custody initialization attempt failed after creating seven empty directories because its `attempt-events` parent was omitted; that failure is preserved. A narrowly predeclared second attempt verified that exact empty partial inventory and completed the remaining approved directories. The guarded runner stops before catalogue or token access because none of the six bound Sentinel sources is promoted and offline-verified. Later `AUX_POEORB` substitution remains separately gated.
...
The unattempted Sentinel runner was corrected before first use so its generated attempt IDs satisfy the lowercase intake identifier grammar while its event timestamps remain RFC 3339. The active intake is unchanged and still waits for a secret-safe credential reference.

The activation-time intake is preserved separately as an immutable snapshot so later append-only progress cannot erase its starting identity. A read-only acquisition-progress validator accepts only the authorized, staging, failed, and promoted states emitted by the runner; binds terminal receipts and promoted byte identity; rejects secret-bearing fields and product drift; and can reconcile external paths and bytes locally without requiring them in public CI. Nine focused tests cover those transitions. The current live result is eight authorized products, zero attempts, and matching empty custody.

A separate checkpoint derivation tool converts only validated eight-product state counts into four explicit outcomes: authentication reference, acquisition in progress, retained-failure review, or container verification. It can emit no-replace candidate profile and goal controls under ignored scratch custody, but it never silently replaces tracked project truth. Nine tests cover every transition, ambiguous counts, source immutability, and candidate-output collision.
