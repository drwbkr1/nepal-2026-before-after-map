...
The current operational gate is orbit recovery-002 implementation publication. Bundle SHA-256 `6d43342b6bda2740667fa6e924a52f15313d8827cfb62563ea107bc483e87fa5` and proposal SHA-256 `d30208c07deb66ef2c7487f8c901abd4fb5ff04aa56766bca8066d4c8d4f0db8` now bind one exact attested approval. That approval releases only the separately tested worker, public CI, one final no-payload preflight, and at most one fresh byte-zero attempt for exact `M2-ORB-001`. It does not release requests for `M2-ORB-002` through `004`, DEM action, radar pixel reads, baseline, change analysis, or scientific publication. A protected owner-controlled credential route obtains a fresh CDSE token and passes it directly through an anonymous single-use pipe without recording its value. Tokens, passwords, cookies, TOTP values, and authorization headers remain excluded from Git, chat, filenames, receipts, environment variables, and captured command output.
...
A separate checkpoint derivation tool converts only validated eight-product state counts and bound control evidence into explicit checkpoints. It can emit no-replace candidate profile and goal controls under ignored scratch custody, but it never silently replaces tracked project truth. The current derivation reaches `M2-ORBIT-RECOVERY-002-REVIEW` only when the radar-first approval, six-source radar readiness, stale-packet preservation, and corrected blank packet all match exactly.
