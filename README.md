...
The next offline transition is also prepared. A gate-deferred materialization runner requires the exact promoted intake and passing container receipt, rechecks archive identity and the complete ZIP namespace, and writes one immutable external SAFE attempt plus a per-file hash manifest. Fourteen synthetic tests cover Windows-safe paths, collision refusal, retained attempts, and pre-custody refusal. A stale production-wrapper test unintentionally materialized `M1-SRC-001` under attempt ID `fixture-must-not-run`; all 26 files and 1,732,324,248 extracted bytes passed the manifest hash check. That append-only attempt is retained as valid materialization only with explicit unintended-test provenance. It establishes no raster readability, usable pixels, baseline, change, or scientific admission, and releases no further processing.
