...
A bounded `.ppkx` portability fixture is now predeclared for that exact metadata-only workspace. It will package, extract, reopen, and re-export once from a fresh non-Git attempt path only after the exact gate passes public CI. Seven portable tests and a local ArcGIS source revalidation pass; no project package has been created yet. Even a later pass will establish only a same-machine ArcGIS Pro 3.7.1 round trip, not clean-machine portability or M6 completion.
