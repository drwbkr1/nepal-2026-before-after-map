...
This goal is **active as of 3 September 2026**. M1 is complete: the search and review AOIs are owner-approved, projected to EPSG:32645, and validated through ArcGIS Pro, and the exact source manifest is approved with eight sources accepted for controlled acquisition and two preserved as deferred context. M2 was activated by an exact owner decision on 3 September 2026. The eight Sentinel products remain unattempted at the secret-safe authentication checkpoint. Separately, four approved Copernicus DEM tiles totaling 170,302,058 bytes were anonymously acquired outside Git and passed exact-byte, ArcGIS structural, and AOI finite-coverage checks. A third independent review now presents four exact S1D restituted orbit files needed by the six radar scenes; it contains zero human decisions and grants no orbit acquisition or application authority. No Sentinel product, vertical-datum conversion, orbit correction, radar result, or scientific change has been established.
...
The parallel `M2-ORBIT-AMENDMENT-REVIEW` checkpoint is ready under bundle SHA-256 `ee5fbf4933b52be8f97441b78a73559a973bd975efc21b43625f1ceca54e2ff1` and proposal SHA-256 `b17e256068759946be611bf4e7beffe0d3121e9e731b6c42163525eca2cf0292`. Official live metadata selected one full-coverage S1D `AUX_RESORB` file for each of four unique acquisition windows, covering the six approved radar sources; no covering `AUX_POEORB` file was available at assessment time. The [orbit amendment review](docs/M2_ORBIT_AMENDMENT_REVIEW.md) authorizes nothing until an exact completed owner response is locked and reconciled.
...
The Sentinel-1 baseline contract fixes independent ascending and descending processing routes, beta-nought calibration, gamma-nought terrain flattening, retained terrain-distortion masks, linear quantitative outputs, and EPSG:32645 delivery. Production processing still defers rather than guessing because the Copernicus DEM uses EGM2008 orthometric heights while ArcGIS's built-in geoid correction is documented as EGM96, and because updated Sentinel orbit files remain additional products outside the active acquisition boundary.
