...
A separate checkpoint derivation tool converts only validated eight-product state counts and container evidence into explicit acquisition or verification checkpoints. It can emit no-replace candidate profile and goal controls under ignored scratch custody, but it never silently replaces tracked project truth. The all-promoted, all-container-pass state advances only to `M2-VERIFY`; it does not release materialization or pixel processing.
