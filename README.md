...
A separate Sentinel-1 input-readiness gate is now predeclared for only those three materialized pre-event sources. Fourteen portable tests and an ArcGIS Pro 3.7.1 synthetic run pass exact-member selection, safe annotation parsing, embedded-orbit ordering and acquisition bracketing, one-band U16 TIFF headers, VV/VH metadata agreement, and a deliberate VV/VH dimension block. It prohibits network access, credentials, pixel decoding, derived rasters, and external mutation. The gate has not yet read a real SAFE; even a future pass can establish only partial pre-event header readiness and cannot release baseline processing.
