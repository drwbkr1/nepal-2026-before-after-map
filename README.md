...
The current checkpoint is `M2-ORBIT-RECOVERY-002-REVIEW`. Bundle SHA-256 `6d43342b6bda2740667fa6e924a52f15313d8827cfb62563ea107bc483e87fa5` binds proposal SHA-256 `d30208c07deb66ef2c7487f8c901abd4fb5ff04aa56766bca8066d4c8d4f0db8` with zero human decisions. Approval would authorize only a separately tested recovery worker, public CI, one final no-payload preflight, and at most one fresh byte-zero attempt for exact `M2-ORB-001`. It would not authorize requests for `M2-ORB-002` through `004`, DEM action, radar pixel reads, baseline, change analysis, or scientific publication. The radar-first approval binds one exact human decision and preserves optical real-001 as `INVALID`, recovery-001 as `BLOCK`, and the old aggregate-dependent orbit packet as stale evidence. A protected owner-controlled credential route can pass a fresh token only through an anonymous single-use pipe without recording its value. Tokens, passwords, cookies, and authorization headers remain excluded from Git, chat, filenames, receipts, and captured command output.
