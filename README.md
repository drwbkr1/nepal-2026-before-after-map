...
This goal is **active as of 7 September 2026**. M1 is complete, and all eight exact Sentinel products are promoted, container verified, and materialized by identity. The optical route remains terminal `INVALID`/`BLOCK` under its fixed criteria. All four approved Sentinel-1D `AUX_RESORB` files are promoted in controlled non-Git custody and pass durable input-only verification. The DEM attempt-003 owner terrain-result review is complete, but the resulting readiness audit remains `DEFER` on vertical and elevation uncertainty, independent accuracy evaluation, and pair-specific radar-input fitness. The owner has selected the exact ArcGIS EGM2008 one-minute preconversion method; the matching optional component is still absent, so the active checkpoint is `M2-DEM-EGM2008-COMPONENT-INSTALL`. No conversion, orbit application, radar-pixel decoding, baseline, change result, attribution, or scientific conclusion has been established.
...
The separately approved DEM amendment covered four exact Copernicus GLO-30 tiles. All four were acquired anonymously through exclusive staging and no-replace promotion, then verified in ArcGIS Pro 3.7.1 as 3600-by-3600, single-band F32 EPSG:4326 rasters. A full scan found 51,840,000 finite non-NoData cells, and the continuous four-tile footprint covers all approved AOI bounds. A later fixed terrain screen passed all four tile checks, four seam checks, AOI slope, EPSG:32645 projection, 189-file stable-manifest reconciliation, and PNG/PDF visual criteria. The owner accepted that exact bounded result after private response lock and reconciliation, so the terrain `human-review` gate passes. The overall audit still defers and establishes only gross-artifact and map-surface fitness. A separate exact owner approval now selects EGM2008 one-minute preconversion to WGS 84 ellipsoidal height followed by `NONE` for verified derivatives. The required optional ArcGIS coordinate-system component is not installed, and independent elevation accuracy, radar processing, and scientific fitness remain unresolved.

The current operational checkpoint is `M2-DEM-EGM2008-COMPONENT-INSTALL`. Corrected continuation commit `ac19e1bd818e86b94bbd35ecf53fe93863ca91f5` passed public CI run `34065851960` before the approved fixed-order acquisition of `M2-ORB-002`, `M2-ORB-003`, and `M2-ORB-004`. Those files and preserved `M2-ORB-001` remain in non-Git custody with SHA-256 values `7462fa65549339c09c36338cb690d0b953c665c3c61a397def3cfa7b2d453262`, `c08bf997f1bf02fb25dcf2a4f35643aef4754fbea03c6c2fb0fa9db4a0efe778`, `281388e94a7a8a708f229fd7d19f93de426bb077699db3aaabdad7125043bd83`, and `a72c93e500a1c09b62b4cd31889837c9d57ccc41542b16397ff9f2c0fccba3f4`, respectively.

The initial four-source verifier invocation evaluated `M2-ORB-001` but stopped at `verification_output_parent_missing`; that result remains terminal indeterminate and is not reconstructed. Recovery proposal SHA-256 `0be64071cfe718ca758155ff0422cfee48af342c8a560528746adc653e6a2fcf` and bundle SHA-256 `d2f0f75f40614c56bc0e62c6eecb1588f7504f50927448192673e6e403b5933f` authorized a pre-read append-only receipt reservation correction. Implementation commit `72d69e787c772ec2157890bc58a8626e7049dce5` passed public CI run `34153008607`; activation and final no-content preflight followed. The new `M2-ORB-001` recovery verification and the existing `002` through `004` attempts then passed exactly once in fixed order, producing four durable `pass_orbit_input_only` receipts without network access, reacquisition, retry, or custody mutation. Terminal reconciliation SHA-256 `42df98bbe1680483d14aa29dedf55f3761bae4be45991a5a84fbb304b9232182` establishes input identity and structural fitness only. Orbit application, vertical-datum resolution, radar pixels, baselines, change analysis, attribution, and scientific publication remain gated.
