...
This goal is **active as of 6 September 2026**. M1 is complete: the search and review AOIs are owner-approved, projected to EPSG:32645, and validated through ArcGIS Pro, and the exact source manifest is approved with eight sources accepted for controlled acquisition and two preserved as deferred context. M2 was activated by an exact owner decision on 3 September 2026. All eight exact Sentinel products are promoted in controlled non-Git custody, passed offline ZIP/SAFE container verification, and are materialized by exact file identity. Both full-cohort header inspections passed without pixel decoding. The first optical pixel-readiness attempt remains terminal `INVALID`; the one approved recovery remains terminal `BLOCK` under the fixed usable-pixel and registration criteria. Orbit recovery-002 is also terminal: one owner handoff reached its detached supervisor, public catalogue revalidation returned, and local setup then failed before an attempt ID, authenticated download, destination, or payload byte. Its one-handoff authority is consumed and cannot be retried. The owner has approved the distinct evidence-first recovery-003 proposal; the exact response is locked and reconciled, and local implementation tests pass. Public CI, activation, the final no-payload preflight, and the owner-selected secret handoff remain pending. Four approved Copernicus DEM tiles remain independently gated by the unresolved vertical-datum and terrain-result reviews. No recovery-003 handoff, catalogue request, orbit payload, radar pixel usability, vertical-datum conversion, orbit correction, baseline, change result, or scientific conclusion has occurred.
...
The current operational gate is public validation of the approved orbit recovery-003 implementation. Recovery-002 public controls passed commit `67ce761003c5c841a068ed688709b348cd423ce3` and CI run `34048602657`, followed by its final no-payload preflight. The subsequent owner handoff reached supervisor `m2-orbit-recovery-002-20260906t172858z-42bace7e`; exact public catalogue revalidation returned, then a local pretransfer setup failure occurred before any attempt ID, authenticated download request, destination, or payload byte. Outcome SHA-256 `3d0b6be8e09e0d9049c32c65461e321b2fdbd5db398dd9a6d1b08b62c6c51380` preserves the bounded known and unknown facts. Recovery-002 is consumed with no retry. Recovery-003 bundle SHA-256 `bc3cdc22d16251c77b26d9903036b4317221e2b01207aa9db26436bfd091fe9d` binds proposal SHA-256 `5aa4a0042024634a7ade191e0c5f36614216d8581a9c0535c0042be20583bfa3`; locked response SHA-256 `0ff1a6e052ae9eaadbea2c55334da54509fe877f9bfd0e637dca4c17eed68804` records the one attested approval. The implementation predeclares its attempt identity, writes attempt-scoped evidence before catalogue access or payload-parent creation, emits fixed nonsecret failure codes, and preserves the independent supervisor journal. It still covers only exact `M2-ORB-001`; the other three orbit files, DEM action, radar pixels, baselines, change analysis, attribution, and scientific publication remain unreleased.
...
A separate checkpoint derivation tool converts only validated eight-product state counts and bound control evidence into explicit checkpoints. It can emit no-replace candidate profile and goal controls under ignored scratch custody, but it never silently replaces tracked project truth. The current derivation reaches `M2-ORBIT-RECOVERY-003-REVIEW` only when the radar-first approval, six-source radar readiness, terminal recovery-002 outcome, consumed authority, and new blank packet all match exactly.
