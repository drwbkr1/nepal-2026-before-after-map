...
This goal is **active as of 5 September 2026**. M1 is complete: the search and review AOIs are owner-approved, projected to EPSG:32645, and validated through ArcGIS Pro, and the exact source manifest is approved with eight sources accepted for controlled acquisition and two preserved as deferred context. M2 was activated by an exact owner decision on 3 September 2026. All eight exact Sentinel products are promoted in controlled non-Git custody, passed offline ZIP/SAFE container verification, and are materialized by exact file identity. Both full-cohort header inspections passed without pixel decoding. The first optical pixel-readiness attempt is terminal `INVALID` before metrics; the separately approved recovery passed its implementation and no-pixel gates, then completed once as terminal `BLOCK` because usable coverage and registration controls did not pass the fixed criteria. The current checkpoint is `M2-RADAR-FIRST-PATH-001-REVIEW`: a blank control-only packet asks whether to preserve and close the optical branch, split route-specific readiness, and prepare a corrected orbit review. Separately, four approved Copernicus DEM tiles totaling 170,302,058 bytes were anonymously acquired outside Git and passed exact-byte, ArcGIS structural, and AOI finite-coverage checks. Four exact S1D restituted orbit identities are approved, but their failed first request remains unretried; the old recovery packet is preserved as stale because its aggregate `M2-VERIFY` prerequisite deadlocks the independent radar route. No radar pixel usability, orbit payload, vertical-datum conversion, orbit correction, baseline, or scientific change has been established.
...
The current checkpoint is `M2-RADAR-FIRST-PATH-001-REVIEW`. Bundle SHA-256 `5a5bd80f724841f9558ad5ff966ed0d49222419f7310b345492172e4639421ad` and proposal SHA-256 `ae2ddfa153a86b7acf7f8ec500690713d5ced9a8ddd58f5655d831e1eb282c77` contain zero human decisions. Approval would authorize only the route-specific control correction and preparation of a new blank orbit review. It would not authorize a token, orbit request, DEM action, radar pixel read, baseline, or change product. A protected owner-controlled credential route previously generated fresh process-local CDSE access tokens and passed each only through an anonymous pipe without recording secret values. After the three initial products and recovery-002 for `M1-SRC-004`, continuation-001 acquired `M1-SRC-005`, `M1-SRC-006`, `M1-SRC-008`, and `M1-SRC-010` exactly once each in the approved order. All eight exact archives now match their recorded local SHA-256 values, passed ZIP/SAFE container verification, and were materialized into append-only verified SAFE roots. Tokens, passwords, cookies, and authorization headers remain excluded from Git, chat, filenames, receipts, and captured command output.
...
The orbit route is retained at a control conflict rather than an actionable acquisition review. The exact decision for review bundle SHA-256 `ee5fbf4933b52be8f97441b78a73559a973bd975efc21b43625f1ceca54e2ff1` and proposal SHA-256 `b17e256068759946be611bf4e7beffe0d3121e9e731b6c42163525eca2cf0292` activated four approved `AUX_RESORB` identities. A stale production-wrapper test later crossed its assumed per-orbit custody guard after `M1-SRC-001` and `M1-SRC-002` became eligible. It used a tracked nonsecret test literal, made a public catalogue request and a rejected download request for exact `M2-ORB-001`, received zero payload bytes, and left no staging or destination payload. The attempt and both external events are retained as a terminal failure. The runner currently requires the entire `M2-VERIFY` unit before catalogue access, token lookup, events, or payload requests.

The old blank orbit recovery review binds bundle SHA-256 `df5aa9d0d03f8ee30a5cd74b91f74a88c83a525e762c22b0bd2b6773ccb5bc6b` to proposal SHA-256 `ce76d633a8104ea5800f51dccd4b1037f930d41b7f08a3de32eed68c6697915a`. It remains exact historical evidence but is no longer presented for approval because its full-`M2-VERIFY` prerequisite cannot be satisfied after the terminal optical block. The other three orbit files remain authorized and unattempted; later `AUX_POEORB` substitution remains separately gated.
