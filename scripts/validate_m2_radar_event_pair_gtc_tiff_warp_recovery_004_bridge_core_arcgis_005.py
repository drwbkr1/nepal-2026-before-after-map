#!/usr/bin/env python3
"""Distinct disposable-005 test of the actual recovery-004 bridge function."""
...
from m2_radar_event_pair_gtc_tiff_warp_recovery_004_core import BridgeStop, bridge_and_warp
...
SCRATCH = ROOT / "scratch/m2-radar-event-area-pair-gtc-tiff-warp-recovery-004-disposable-005"
RECEIPT = ROOT / "records/readiness/m2-radar-event-area-pair-gtc-tiff-warp-recovery-004-arcgis-disposable-005.json"
...
def run() -> dict:
    ...
    if SCRATCH.exists() or RECEIPT.exists():
        raise RuntimeError("append_only_disposable_005_collision")
    ...
    try:
        ...
        code = str(exc) if type(exc) in (RuntimeError, BridgeStop) else "disposable_core_exception"
