#!/usr/bin/env python3
"""Append-only ArcGIS CRF and per-materialization resource test."""
...
ATTEMPT = ROOT / "scratch/map-route-feasibility-disposable-004"
...
PUBLIC_RECEIPT = ROOT / "records/readiness/m2-map-route-feasibility-001-arcgis-disposable-004.json"
...
def run() -> dict:
    ...
    try:
        ...
        for name, categorical, bands in (("continuous", False, 2), ("categorical", True, 1)):
            ...
            def before_materialization(operation):
                free = shutil.disk_usage(ATTEMPT).free
                if free < 40 * 1024**3:
                    raise RuntimeError("disposable_materialization_free_space_below_40_gib")
                materialization_stages.append(operation)
            ...
            if materialization_stages != ["ProjectRaster", "Clip"]:
                raise RuntimeError("materialization_stage_order_drift")
            result["per_materialization_resource_stages"] = materialization_stages
    ...
    public = {"schema_version": "1.0", "record_id": "NEPAL-M2-MAP-ROUTE-FEASIBILITY-001-ARCGIS-DISPOSABLE-004",
