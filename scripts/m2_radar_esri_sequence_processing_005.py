#!/usr/bin/env python3
"""Recovery-005 processing with Esri's documented RTF, Despeckle, then GTC sequence."""
...
def arcgis_signature_status(arcpy: Any) -> dict[str, Any]:
    expected = {
        ...
        "Despeckle": (
            "in_radar_data", "polarization_bands", "filter_type", "filter_size", "noise_model",
            "noise_variance", "add_noise_mean", "mult_noise_mean", "number_of_looks", "damp_factor",
        ),
    ...
    observed = {
        name: tuple(inspect.signature(getattr(arcpy.ia, name)).parameters)
        for name in expected
    }
    return {"parameters": observed, "all_match": observed == expected}
...
def run_final_preflight(checked_at_utc: str) -> int:
    ...
    receipt = {
        "schema_version": "1.0",
        "receipt_id": "NEPAL-M2-RADAR-RASTER-FUNCTION-CALL-SHAPE-RECOVERY-004-FINAL-PREFLIGHT",
...
def _save_raster_result(
    *,
    ...
    output: Path,
    source_id: str,
    stage_recorder: Callable[..., None],
) -> None:
    if output.exists():
        raise RadarRouteError("raster_function_output_collision", f"{source_id}:{tool_name}")
    stage_recorder(source_id=source_id, tool=tool_name, phase="started")
    result = function(*arguments)
    save = getattr(result, "save", None)
    if not callable(save):
        raise RadarRouteError("raster_function_result_missing_save", f"{source_id}:{tool_name}")
    save(str(output))
    if not output.exists():
        raise RadarRouteError("raster_function_output_missing_after_save", f"{source_id}:{tool_name}")
    stage_recorder(source_id=source_id, tool=tool_name, phase="completed")


def execute_raster_function_pipeline(
    *,
    arcpy: Any,
    manifest: Path,
    dem_mosaic: Path,
    ...
    _save_raster_result(
        tool_name="RemoveThermalNoise",
        function=arcpy.ia.RemoveThermalNoise,
        arguments=(str(manifest), "VV;VH"),
        output=outputs["thermal"],
        source_id=source_id,
        stage_recorder=stage_recorder,
    )
    _save_raster_result(
        tool_name="ApplyRadiometricCalibration",
        function=arcpy.ia.ApplyRadiometricCalibration,
        arguments=(str(outputs["thermal"]), "VV;VH", "BETA_NOUGHT"),
        output=outputs["beta"],
        source_id=source_id,
        stage_recorder=stage_recorder,
    )
    _save_raster_result(
        tool_name="ApplyRadiometricTerrainFlattening",
        function=arcpy.ia.ApplyRadiometricTerrainFlattening,
        arguments=(
            str(outputs["beta"]),
            str(dem_mosaic),
            "NONE",
            "VV;VH",
            "GAMMA_NOUGHT",
            str(outputs["scattering"]),
            str(outputs["distortion"]),
            str(outputs["native_mask"]),
        ),
        output=outputs["gamma_slant"],
        source_id=source_id,
        stage_recorder=stage_recorder,
    )
    _save_raster_result(
        tool_name="Despeckle",
        function=arcpy.ia.Despeckle,
        arguments=(str(outputs["gamma_slant"]), "VV;VH", "REFINED_LEE"),
        output=outputs["gamma_despeckled"],
        source_id=source_id,
        stage_recorder=stage_recorder,
    )
    with arcpy.EnvManager(
        snapRaster=str(support["snap"]), cellSize=10.0,
        resamplingMethod="BILINEAR", overwriteOutput=False,
    ):
        _save_raster_result(
            tool_name="ApplyGeometricTerrainCorrection_gamma",
            function=arcpy.ia.ApplyGeometricTerrainCorrection,
            arguments=(str(outputs["gamma_despeckled"]), "VV;VH", str(dem_mosaic), "NONE"),
            output=outputs["gamma_gtc_raw"],
            source_id=source_id,
            stage_recorder=stage_recorder,
        )
    _save_raster_result(
        tool_name="ConvertSARUnits",
        function=arcpy.ia.ConvertSARUnits,
        arguments=(str(outputs["gamma_gtc_raw"]), "LINEAR_TO_DB"),
        output=outputs["db_gtc_raw"],
        source_id=source_id,
        stage_recorder=stage_recorder,
    )
    with arcpy.EnvManager(
        snapRaster=str(support["snap"]), cellSize=10.0,
        resamplingMethod="NEAREST", overwriteOutput=False,
    ):
        _save_raster_result(
            tool_name="ApplyGeometricTerrainCorrection_mask",
            function=arcpy.ia.ApplyGeometricTerrainCorrection,
            arguments=(str(outputs["native_mask"]), "#", str(dem_mosaic), "NONE"),
            output=outputs["mask_gtc_raw"],
            source_id=source_id,
            stage_recorder=stage_recorder,
        )
...
def process_source(
    *,
    ...
    try:
        ...
        execute_raster_function_pipeline(
            arcpy=arcpy,
            manifest=manifest,
    ...
    try:
        ...
        receipt = {
            ...
            "assertions": {
                ...
                "primary_despeckle_applied": True,
                "primary_despeckle_filter": "REFINED_LEE",
...
def run_execute(executed_at_utc: str) -> int:
    ...
    try:
        ...
        terminal = {
            "schema_version": "1.0",
            "receipt_id": "NEPAL-M2-RADAR-RASTER-FUNCTION-CALL-SHAPE-RECOVERY-004-TERMINAL-RECONCILIATION",
