#!/usr/bin/env python3
"""Record public CI for the diagnostic receipt-recovery execution gate."""
...
NEXT_ACTION = (
    "Run the one authorized final no-content preflight once. Stop on failure; only on pass may the one distinct read-only "
    "diagnostic process begin in ArcGIS Pro Python, and it must never be retried."
)
...
def main() -> int:
    ...
    if (
        ...
        or reconciliation.get("status") != "pass_public_implementation_gate_execution_gate_publication_pending"
    ):
        raise SystemExit("implementation publication gate differs")

    record = {
        "schema_version": "1.0",
        "record_id": "NEPAL-M2-RADAR-APPLY-ORBIT-CORRECTION-INPUT-RESOLUTION-DIAGNOSTIC-RECEIPT-PERSISTENCE-RECOVERY-001-GATE-STATE-PUBLICATION",
        "recorded_at_utc": args.recorded_at_utc,
        "status": "pass_public_execution_gate_final_no_content_preflight_released",
        ...
        "released_now": {"final_no_content_preflight": True, "distinct_process_only_on_preflight_pass": True},
        "assertions": {
    ...
    execution["gates"].update({
        "execution_gate_publication": "success",
        "gate_state_commit_sha": args.gate_state_commit_sha,
    ...
    evidence = {
        "record_id": "EVID-0202",
        "type": "m2_radar_apply_orbit_correction_input_resolution_diagnostic_receipt_persistence_recovery_001_gate_state_publication",
        "verified_at_utc": args.recorded_at_utc,
        "status": record["status"],
        "claim": "The exact execution-gate state passed public default-branch CI. One final no-content preflight is released; the distinct read-only process remains conditional on that pass.",
