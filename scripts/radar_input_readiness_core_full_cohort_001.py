#!/usr/bin/env python3
"""Portable decisions for the six-source read-only Sentinel-1 SAFE readiness gate."""
...
def validate_contract(contract: dict[str, Any]) -> list[str]:
    errors: list[str] = []
    if contract.get("contract_id") != "NEPAL-S1-MATERIALIZED-INPUT-READINESS-003":
        errors.append("contract identity differs")
    if contract.get("status") != "active_full_cohort_001_exact_six_sources":
        errors.append("contract status differs")
    if contract.get("metadata_checks", {}).get("pixel_value") != "Detected":
        errors.append("amended pixel value label differs")
    ...
    if not isinstance(sources, list) or [item.get("source_id") for item in sources] != [
        ...
        expected_products = [
            ...
            "S1D_IW_GRDH_1SDV_20260828T122116_20260828T122141_004326_007FA4_C523.SAFE",
            "S1D_IW_GRDH_1SDV_20260828T122141_20260828T122206_004326_007FA4_01B4.SAFE",
            "S1D_IW_GRDH_1SDV_20260831T001037_20260831T001102_004362_0080EC_2C5B.SAFE",
        ]
        ...
        if [item.get("event_role") for item in sources] != ["before", "before", "before", "after", "after", "after"]:
            errors.append("before/after source roles differ")
    ...
    if decision.get("all_six_pass_status") != "pass_full_radar_header_readiness_only":
        errors.append("full-cohort readiness decision differs")
...
def validate_annotation(
    ...
    for key in ("range_pixel_spacing", "azimuth_pixel_spacing"):
        ...
        if not isinstance(value, (int, float)) or isinstance(value, bool) or not math.isfinite(float(value)) or value <= 0:
            errors.append(f"{polarization} annotation {key} is invalid")
    expected_pixel_value = str(contract["metadata_checks"]["pixel_value"])
    if str(parsed.get("pixel_value", "")) != expected_pixel_value:
        errors.append(f"{polarization} annotation pixel value is not {expected_pixel_value}")
...
def summarize_full_readiness(source_decisions: dict[str, dict[str, Any]]) -> dict[str, Any]:
    expected = ["M1-SRC-001", "M1-SRC-002", "M1-SRC-003", "M1-SRC-004", "M1-SRC-005", "M1-SRC-006"]
    errors = []
    ...
    return {
        "status": "pass_full_radar_header_readiness_only" if not errors else "block",
        "errors": errors,
        "ready_source_count": 6 - len(errors),
        "complete_before_after_pair": not errors,
        "baseline_processing_released": False,
