...
$brokerPath = Join-Path $PSScriptRoot 'm2_orbit_recovery_002_broker.py'
...
try {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw 'orbit_recovery_broker_failed'
    }
}
catch {
    Write-Error ('Secret-safe orbit recovery stopped: {0}' -f $_.Exception.Message)
    exit 12
}
finally {
    $accessToken = $null
