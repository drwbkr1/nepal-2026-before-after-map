...
PUBLICATION_CHECKPOINT = "M2-RADAR-APPLY-ORBIT-CORRECTION-INPUT-RESOLUTION-DIAGNOSTIC-RECEIPT-PERSISTENCE-RECOVERY-001-REVIEW-PUBLICATION"
CHECKPOINT = "M2-RADAR-APPLY-ORBIT-CORRECTION-INPUT-RESOLUTION-DIAGNOSTIC-RECEIPT-PERSISTENCE-RECOVERY-001-REVIEW"
UNIT_ID = "M2-RADAR-APPLY-ORBIT-CORRECTION-INPUT-RESOLUTION-DIAGNOSTIC-RECEIPT-PERSISTENCE-RECOVERY-001-REVIEW"
NEXT_ACTION = (
    "Review the exact public M2 ApplyOrbitCorrection input-resolution diagnostic receipt-persistence recovery-001 "
    "bundle and proposal; approve, revise, or defer the bounded receipt-durability correction and one distinct future "
    "read-only process. No implementation, ArcPy invocation, project-data or external-custody access, new process, "
    "retry, reconstruction, mutation of either consumed receipt, geoprocessing, radar processing, baseline or change "
    "analysis, attribution, or scientific publication is authorized before an exact attested decision."
)
...
def main() -> int:
    ...
    if args.required_file_count <= 0 or args.public_test_count <= 0 or args.public_skip_count < 0:
        raise SystemExit("public validation counts are invalid")
    ...
    if (
        ...
        or review.get("gates", {}).get("new_diagnostic_process_authorized") is not False
        or review.get("gates", {}).get("reserved_receipt_mutation_authorized") is not False
    ...
    gate = {
        "schema_version": "1.0",
        "record_id": "NEPAL-M2-RADAR-APPLY-ORBIT-CORRECTION-INPUT-RESOLUTION-DIAGNOSTIC-RECEIPT-PERSISTENCE-RECOVERY-001-REVIEW-PUBLICATION-GATE",
    ...
    reconciliation = {
        "schema_version": "1.0",
        "record_id": "NEPAL-M2-RADAR-APPLY-ORBIT-CORRECTION-INPUT-RESOLUTION-DIAGNOSTIC-RECEIPT-PERSISTENCE-RECOVERY-001-REVIEW-PUBLICATION-RECONCILIATION",
    ...
    evidence = {
        "record_id": "EVID-0198",
        "type": "m2_radar_apply_orbit_correction_input_resolution_diagnostic_receipt_persistence_recovery_001_review_publication_gate",
        "verified_at_utc": args.verified_at_utc,
        "status": "pass_public_ci_zero_decision_owner_review_ready",
        "claim": "The exact blank diagnostic receipt-persistence recovery packet passed public default-branch CI. Only owner proposal review is released; the consumed diagnostic and two zero-byte reserved receipts remain immutable; and no implementation, ArcPy, data access, new process, retry, reconstruction, geoprocessing, radar processing, baseline, change analysis, attribution, or scientific publication occurred.",
        "publication_gate_sha256": sha256(GATE_REF),
        "publication_reconciliation_sha256": sha256(RECONCILIATION_REF),
        "proposal_sha256": PROPOSAL_SHA256,
        "review_bundle_sha256": BUNDLE_SHA256,
