#!/usr/bin/env python3
"""Record public CI for the diagnostic execution-gate state."""
...
from m2_radar_apply_orbit_correction_input_resolution_diagnostic_001_core import ATTEMPT_ID, canonical_bytes, sha256_file, write_new_json
...
NEXT_ACTION = (
    "Run the one authorized final no-content preflight once. Stop on failure; only on pass may the exact one-process "
    "read-only diagnostic inspect the frozen recovery-002 candidate paths and M2-ORB-001. Never retry it."
)
...
def main() -> int:
    ...
    for ref in (f"records/readiness/{PREFIX}-final-preflight.json", f"records/processing/{PREFIX}-terminal.json", f"records/processing/{PREFIX}-cleanup.json"):
        if (ROOT / ref).exists():
    ...
    record = {
        "schema_version": "1.0",
        "record_id": "NEPAL-M2-RADAR-APPLY-ORBIT-CORRECTION-INPUT-RESOLUTION-DIAGNOSTIC-001-GATE-STATE-PUBLICATION",
        "recorded_at_utc": args.recorded_at_utc,
        "status": "pass_public_gate_state_final_no_content_preflight_released",
        ...
        "released_now": {"final_no_content_preflight": True, "diagnostic_process_only_on_preflight_pass": True},
        "assertions": {
    ...
    evidence = {
        "record_id": "EVID-0193",
        "type": "m2_radar_apply_orbit_correction_input_resolution_diagnostic_001_gate_state_publication",
        "verified_at_utc": args.recorded_at_utc,
        "status": record["status"],
        "claim": "The exact diagnostic execution-gate state passed public default-branch CI. One final no-content preflight is released; the read-only diagnostic remains conditional on that pass.",
