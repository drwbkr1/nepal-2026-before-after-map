#!/usr/bin/env python3
"""Continue one approved M2 product using the supervisor's in-memory token."""
...
from acquire_m2_sentinel_recovery_002 import stream_to_exclusive_staging_with_progress
from m2_sentinel_recovery_002_core import (
    CONTRACT_REF as RECOVERY_CONTRACT_REF,
    EXPECTED_SOURCE_ID as RECOVERY_SOURCE_ID,
    SECRET_REFERENCE,
...
ALLOWED_SOURCE_IDS = {"M1-SRC-005", "M1-SRC-006", "M1-SRC-008", "M1-SRC-010"}
RECOVERY_CONTRACT_PATH = ROOT / RECOVERY_CONTRACT_REF
...
def run_product(
    ...
    *,
    ...
    if source_id not in ALLOWED_SOURCE_IDS:
        raise Recovery002ControlError("continuation_source_outside_exact_release")
    ...
    recovery_successes = [item for item in recovery_asset.get("attempts", []) if item.get("outcome") == "succeeded"]
    if recovery_asset.get("state") != "promoted" or len(recovery_successes) != 1:
        raise TransferControlError("recovery_002_not_successfully_promoted")
    recovery_attempt_id = recovery_successes[0]["attempt_id"]
    recovery_container_path = CONTAINER_ROOT / f"{RECOVERY_SOURCE_ID.casefold()}-{recovery_attempt_id}.json"
    if not recovery_container_path.is_file() or load(recovery_container_path).get("status") != "pass_container_only":
        raise TransferControlError("recovery_002_container_gate_not_passing")
    if preflight.get("status") != "pass_no_external_mutation":
        raise TransferControlError("m2_preflight_not_passing")
    matching_assets = [item for item in intake["assets"] if item.get("extensions", {}).get("source_id") == source_id]
    matching_plan = [item for item in plan["records"] if item.get("source_id") == source_id]
    ...
    expected_source_uri = f"https://{DOWNLOAD_HOST}/odata/v1/Products({asset['extensions']['provider_product_id']})/$value"
    if source_uri != expected_source_uri or parsed_source.query or parsed_source.fragment:
    ...
    if progress:
        progress("continuation_live_preflight", None, 0)
    ...
    if progress:
        progress("continuation_transfer", attempt_id, 0)
    ...
    try:
        ...
        with opener.open(request, timeout=120) as response:
            ...
            staged = stream_to_exclusive_staging_with_progress(
                ...
                progress_callback=(
                    (lambda size: progress("continuation_transfer", attempt_id, size))
                    if progress else None
                ),
            )
        if progress:
            progress("continuation_promotion", attempt_id, staged["size_bytes"])
...
if __name__ == "__main__":
    raise SystemExit("This module accepts credentials only from the detached recovery-002 supervisor in memory.")
