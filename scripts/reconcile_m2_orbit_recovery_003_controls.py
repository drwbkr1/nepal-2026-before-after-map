#!/usr/bin/env python3
"""Reconcile the exact approved orbit recovery-003 into active project controls after public CI."""
...
OUTPUT = ROOT / "records/readiness/m2-orbit-recovery-003-control-reconciliation.json"
CHECKPOINT = "M2-ORBIT-RECOVERY-003"
...
def main() -> int:
    ...
    review = _unit(milestone, "M2-ORBIT-RECOVERY-003-REVIEW")
    ...
    if initial_state == ("ready", "planned", "planned"):
        review.update({
            ...
            "next_dependency": "M2-ORBIT-RECOVERY-003-IMPLEMENTATION",
        })
        review["gates"].update({
        ...
        raise SystemExit("orbit recovery-003 milestone is not at a supported reviewed or pre-attempt correction checkpoint")
    implementation.update({
        "status": "complete",
        "disposition": "pass",
        "inputs": [APPROVAL_REF, "records/acquisition/m2-orbit-recovery-003-implementation-readiness.json"],
    ...
    implementation["exit_condition_delta"] = {
        ...
        "decision_value": "enables_dependency",
        "rationale": "The exact evidence-first recovery-only implementation passed local controls and public CI without credentials or orbit bytes.",
    }
    recovery.update({
        "status": "ready",
        "disposition": None,
        "inputs": [PUBLICATION_GATE_REF, "contracts/m2-orbit-recovery-003.json", "records/acquisition/m2-orbit-recovery-003-final-preflight.json"],
    ...
    acquire["gates"]["retained_failure_review"] = "approved_exact_one_file_recovery_003"
    acquire["gates"]["orbit_recovery_003_status"] = "ready_final_no_payload_preflight_pending"
    milestone["handoff"]["current_checkpoint"] = CHECKPOINT
    milestone["handoff"]["next_action"] = NEXT_ACTION
    carry_forward = "Orbit recovery-003 authorizes one owner handoff and at most one byte-zero M2-ORB-001 request only after final preflight; it authorizes no later orbit source request or retry."
    ...
    matching_gates = [
        ...
        if item.get("unit_id") == "M2-ORBIT-RECOVERY-003-REVIEW"
    ]
    if len(matching_gates) != 1:
        raise SystemExit("recovery-003 profile gate missing or ambiguous")
    matching_gates[0].update({
        "reason": "The exact owner decision authorizes only the evidence-first recovery-003 implementation, public-CI gate, final no-payload preflight, one owner-selected handoff, and at most one M2-ORB-001 request.",
