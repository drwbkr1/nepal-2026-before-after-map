#!/usr/bin/env python3
"""Distinct recovery-004 source processor; old attempt code stays unchanged."""
...
def _verify_dem_native_extent(dem_mosaic: Path, bounds: list[float]) -> dict[str, Any]:
    """Conservative actual native grid rectangle; any missing DEM cell stops."""
    import numpy as np  # type: ignore
    from osgeo import gdal, ogr  # type: ignore

    gdal.UseExceptions()
    gdal.SetConfigOption("PROJ_NETWORK", "OFF")
    dataset = gdal.OpenEx(str(dem_mosaic), gdal.OF_RASTER | gdal.OF_READONLY)
    if dataset is None:
        raise NativeGridStop("actual_native_extent_dem_unreadable")
    try:
        west, south, east, north = bounds
        ring = [(west, south), (east, south), (east, north), (west, north)]
        polygon = polygon_from_ring(ring, ogr)
        result = scan_dem_validity(dataset, [polygon, polygon], gdal=gdal, ogr=ogr, np=np)
        if result["invalid_dem_cells"] != 0 or result["valid_dem_cells"] <= 0:
            raise NativeGridStop("actual_native_extent_dem_gap")
        result["conservative_full_native_grid_rectangle"] = True
        result["bbox_frame_is_valid_sar_pixel_proof"] = False
        return result
    finally:
        dataset.Close()
...
def _save_native_gtc(*, arcpy: Any, arguments: tuple[Any, ...], output: Path,
                     ...
                     attempt_root: Path, dem_mosaic: Path,
    ...
    with native_gtc_environment(arcpy, resampling="NEAREST" if categorical else "BILINEAR",
        ...
        metadata["actual_extent_dem_support"] = _verify_dem_native_extent(dem_mosaic, metadata["bounds"])
        write_new_json(event_dir / f"{source_id.lower()}-{name}-dem-support.json",
                       metadata["actual_extent_dem_support"])
        stage_recorder(source_id=source_id, tool=name, phase="actual_extent_dem_support_passed")
...
def execute_raster_function_pipeline(*, arcpy: Any, manifest: Path, dem_mosaic: Path,
    ...
    gamma = _save_native_gtc(
        ...
        attempt_root=attempt_root, dem_mosaic=dem_mosaic,
        source_bounds=source_bounds, aoi_bounds=aoi_bounds,
        categorical=False)
...
def process_source(
    *,
    ...
    try:
        ...
        gamma_tiff = source_root / "gamma0_linear_native_bridge.tif"
        db_tiff = source_root / "gamma0_db_native_bridge.tif"
        mask_tiff = source_root / "geometric_distortion_mask_native_bridge.tif"
        gamma_final = source_root / "gamma0_linear_epsg32645_10m.tif"
        db_final = source_root / "gamma0_db_epsg32645_10m.tif"
        mask_final = source_root / "geometric_distortion_mask_epsg32645_10m.tif"
        ...
        projected_grids = {}
        for name, native, bridge, final, categorical in (
                ("gamma", gamma_gtc_raw, gamma_tiff, gamma_final, False),
                ("db", db_gtc_raw, db_tiff, db_final, False),
                ("mask", mask_gtc_raw, mask_tiff, mask_final, True)):
            stage_recorder(source_id=source_id, tool=f"CopyRaster_Warp_{name}", phase="started")
            projected_grids[name] = bridge_and_warp(
                arcpy, native, bridge, final, categorical=categorical)
            stage_recorder(source_id=source_id, tool=f"CopyRaster_Warp_{name}", phase="completed")
