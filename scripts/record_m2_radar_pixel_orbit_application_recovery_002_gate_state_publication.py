#!/usr/bin/env python3
"""Record public CI for the recovery-002 execution-gate state."""
...
from m2_radar_pixel_orbit_application_recovery_002_core import CONTRACT_REF, write_new_json
from run_m2_radar_pixel_orbit_application_recovery_002 import ATTEMPT_ID, repository_bindings
...
NEXT_ACTION = (
    "Run the one authorized final no-content preflight once. Stop on failure; only on pass may the single-process "
    "fresh recovery-002 attempt read the six exact sources, four exact orbit files, four exact DEM derivatives, and "
    "evaluate the two frozen routes. The attempt must never be retried."
)
...
def main() -> int:
    ...
    record = {
        "schema_version": "1.0",
        "record_id": "NEPAL-M2-RADAR-PIXEL-ORBIT-APPLICATION-RECOVERY-002-GATE-STATE-PUBLICATION",
        "recorded_at_utc": args.recorded_at_utc,
        "status": "pass_public_default_branch_ci_one_final_preflight_released",
        ...
        "released_now": {"final_no_content_preflight": True, "fresh_attempt_only_on_preflight_pass": True},
        "assertions": {
    ...
    evidence = {
        "record_id": "EVID-0186",
        "type": "m2_radar_pixel_orbit_application_recovery_002_gate_state_publication",
        "verified_at_utc": args.recorded_at_utc,
        "status": "pass_public_gate_state_final_preflight_released",
        "claim": "The exact recovery-002 execution gate state passed public default-branch CI. One final no-content preflight is released; the fresh attempt remains conditional on that pass.",
