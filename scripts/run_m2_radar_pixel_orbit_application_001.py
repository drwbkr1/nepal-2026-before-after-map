#!/usr/bin/env python3
"""Run the single approved final preflight or real radar processing attempt."""
...
def process_source(
    *,
    ...
    started_ref = source_receipt_ref(source_id, "started")
    terminal_ref = source_receipt_ref(source_id, "terminal")
    source_root = attempt_root / "sources" / source_id.lower()
    write_new_json(
        ROOT / started_ref,
        {
            "schema_version": "1.0",
            "receipt_id": f"NEPAL-{source_id}-RADAR-PIXEL-ORBIT-APPLICATION-001-STARTED",
    ...
    try:
        ...
        receipt = {
            "schema_version": "1.0",
            "receipt_id": f"NEPAL-{source_id}-RADAR-PIXEL-ORBIT-APPLICATION-001-TERMINAL",
        ...
        write_new_json(ROOT / terminal_ref, receipt)
        return {"source_id": source_id, "status": receipt["status"], "receipt_ref": terminal_ref, "receipt_sha256": repo_sha(terminal_ref)}
        ...
        return {"source_id": source_id, "status": failure["status"], "receipt_ref": terminal_ref, "receipt_sha256": repo_sha(terminal_ref)}
...
def process_route(
    *,
    ...
    started_ref = route_receipt_ref(route_id, "started")
    terminal_ref = route_receipt_ref(route_id, "terminal")
    write_new_json(
        ROOT / started_ref,
        {
            "schema_version": "1.0",
            "receipt_id": f"NEPAL-{route_id}-RADAR-PIXEL-QA-001-STARTED",
    ...
    try:
        def paths(source_ids: list[str], name: str) -> list[Path]:
            return [attempt_root / "sources" / source_id.lower() / name for source_id in source_ids]
        ...
        receipt = {
            "schema_version": "1.0",
            "receipt_id": f"NEPAL-{route_id}-RADAR-PIXEL-QA-001-TERMINAL",
        ...
        write_new_json(ROOT / terminal_ref, receipt)
        return {"route_id": route_id, "status": receipt["status"], "disposition": result["status"], "receipt_ref": terminal_ref, "receipt_sha256": repo_sha(terminal_ref)}
    except Exception as exc:
        failure = {
            "schema_version": "1.0",
            "receipt_id": f"NEPAL-{route_id}-RADAR-PIXEL-QA-001-TERMINAL",
        ...
        write_new_json(ROOT / terminal_ref, failure)
        return {"route_id": route_id, "status": failure["status"], "receipt_ref": terminal_ref, "receipt_sha256": repo_sha(terminal_ref)}
