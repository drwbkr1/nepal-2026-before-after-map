#!/usr/bin/env python3
"""Validate the six-source Sentinel-1 input-readiness rules with synthetic ArcGIS TIFFs."""
...
from inspect_radar_inputs_arcgis_full_cohort_001 import describe_raster
from m2_materialization_core import write_new_json
from radar_input_readiness_core_full_cohort_001 import (
    ROLE_PATTERNS,
    decide_source_readiness,
    parse_s1_annotation,
    select_required_members,
    summarize_full_readiness,
    validate_contract,
)


ROOT = Path(__file__).resolve().parents[1]
CONTRACT_REF = "config/qa/radar-input-readiness-contract-full-cohort-001.json"
...
<product>
  ...
  <imageAnnotation><imageInformation><numberOfSamples>{width}</numberOfSamples><numberOfLines>{height}</numberOfLines>
    <pixelValue>Detected</pixelValue><outputPixels>16 bit Unsigned Integer</outputPixels>
...
def main() -> int:
    ...
    aggregate = summarize_full_readiness(source_decisions)
    ...
    status = "pass_synthetic_arcgis_real_input_deferred" if (
        aggregate["status"] == "pass_full_radar_header_readiness_only"
        and mismatch["status"] == "block"
    ) else "fail"
    receipt = {
        "receipt_version": "1.0",
        "receipt_id": "NEPAL-S1-MATERIALIZED-INPUT-READINESS-SYNTHETIC-FULL-COHORT-001",
        "verified_at_utc": args.verified_at_utc,
        "status": status,
        "bindings": {
            "contract_ref": CONTRACT_REF,
            "contract_sha256": digest(CONTRACT_REF),
            "core_ref": "scripts/radar_input_readiness_core_full_cohort_001.py",
            "core_sha256": digest("scripts/radar_input_readiness_core_full_cohort_001.py"),
            "runner_ref": "scripts/inspect_radar_inputs_arcgis_full_cohort_001.py",
            "runner_sha256": digest("scripts/inspect_radar_inputs_arcgis_full_cohort_001.py"),
            "adapter_ref": "scripts/validate_radar_input_readiness_arcgis_full_cohort_001.py",
            "adapter_sha256": digest("scripts/validate_radar_input_readiness_arcgis_full_cohort_001.py"),
        },
        "runtime": {
        ...
        "validation": {
            "synthetic_source_count": 6,
            "synthetic_measurement_raster_count": 12,
        ...
        "next_action": "Publish this amended gate and require successful public CI before the one authorized real-003 inspection.",
    }
