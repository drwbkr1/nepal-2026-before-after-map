#!/usr/bin/env python3
"""Run the one approved stage-evidenced radar short-path recovery-003 attempt."""
...
import m2_radar_short_path_processing_003 as short_runner
from m2_radar_short_path_recovery_003_core import (
    ATTEMPT_ID,
    BASE_CONTRACT_REF,
    CONTRACT_REF,
    ROUTE_ORDER,
...
SHORT_PROCESSING_REF = "scripts/m2_radar_short_path_processing_003.py"
...
def repository_bindings() -> dict[str, str]:
    return {
        ...
        "short_path_processing_sha256": repo_sha(SHORT_PROCESSING_REF),
    }
...
def publication_gate() -> dict[str, Any]:
    ...
    if not path.is_file():
        raise base_core.RadarRouteError("short_path_recovery_003_implementation_publication_gate_missing")
    gate = base_core.load_object(path)
    if (
        gate.get("status") != "pass_public_default_branch_ci_short_path_recovery_003_implementation_ready"
        ...
        raise base_core.RadarRouteError("short_path_recovery_003_implementation_publication_gate_not_exact_pass")
    return gate
...
def gate_state_publication() -> dict[str, Any]:
    ...
    if not path.is_file():
        raise base_core.RadarRouteError("short_path_recovery_003_gate_state_publication_missing")
    ...
    if (
        ...
        raise base_core.RadarRouteError("short_path_recovery_003_gate_state_publication_not_exact_pass")
    return state
...
def build_final_preflight(checked_at_utc: str) -> dict[str, Any]:
    ...
    if target.exists() or (ROOT / TERMINAL_REF).exists():
        raise base_core.RadarRouteError("short_path_recovery_003_attempt_or_terminal_collision")
    if is_subst_drive(target):
        raise base_core.RadarRouteError("short_path_subst_drive_prohibited")
    if path_chain_has_reparse(target):
        raise base_core.RadarRouteError("short_path_reparse_point_prohibited")
    ...
    return {
        "schema_version": "1.0",
        "receipt_id": "NEPAL-M2-RADAR-PIXEL-ORBIT-APPLICATION-RECOVERY-002-FINAL-PREFLIGHT",
...
def _reserve_attempt(target: Path, executed_at_utc: str, bindings: dict[str, Any]) -> tuple[Path, Path]:
    if target.exists():
        raise base_core.RadarRouteError("short_path_recovery_003_attempt_collision")
    ...
    terminal_reservation = {
        "schema_version": "1.0",
        "reservation_id": "NEPAL-M2-RADAR-PIXEL-ORBIT-APPLICATION-RECOVERY-002-TERMINAL-RESERVATION",
    ...
    cleanup_reservation = {
        "schema_version": "1.0",
        "reservation_id": "NEPAL-M2-RADAR-PIXEL-ORBIT-APPLICATION-RECOVERY-002-CLEANUP-RESERVATION",
        "reserved_at_utc": executed_at_utc,
        "attempt_id": ATTEMPT_ID,
    ...
    write_new_json(target / "started.json", {
        "schema_version": "1.0",
        "status": "started_single_stage_evidenced_short_path_recovery_003_attempt",
...
def execute_attempt_body(
    ...
    *,
    ...
    terminal_status = "block_short_path_recovery_003_failure_no_retry"
    ...
    try:
        ...
        terminal_status = (
            ...
            else "block_short_path_recovery_003_failure_no_retry"
        )
        try:
            terminal: dict[str, Any] = {
                "schema_version": "1.0",
                "receipt_id": "NEPAL-M2-RADAR-PIXEL-ORBIT-APPLICATION-RECOVERY-002-TERMINAL",
        ...
        try:
            cleanup = {
                "schema_version": "1.0",
                "receipt_id": "NEPAL-M2-RADAR-PIXEL-ORBIT-APPLICATION-RECOVERY-002-CLEANUP",
