#!/usr/bin/env python3
"""Run the one authorized six-source Sentinel-1 read-only real-003 inspection."""
...
def main() -> int:
    ...
    if args.receipt_output != "records/readiness/radar-input/m2-s1-input-readiness-real-003.json":
        stop("unexpected_real_003_receipt_identity")
    ...
    for ref_key, hash_key in (
        ...
        ("materialization_reconciliation_ref", "materialization_reconciliation_sha256"),
    ...
    milestone = load_json(ROOT / "contracts/milestone-002.json")
    ...
    summary = summarize_full_readiness(source_decisions)
    ...
    all_six_ready = summary["status"] == "pass_full_radar_header_readiness_only"
    receipt = {
        "receipt_version": "1.0",
        "receipt_id": "NEPAL-S1-MATERIALIZED-INPUT-READINESS-REAL-003",
        ...
        "claim_boundary": {
            "exact_six_source_member_and_header_readiness_established": all_six_ready,
            "pixel_values_examined": False,
            "pixel_usability_established": False,
            "complete_pair_established": all_six_ready,
            "baseline_established": False,
            "change_established": False,
            "scientific_admission_authorized": False,
        },
        "limitations": contract["limitations"],
        "next_gate": "independent radar pixel readiness remains gated on orbit and DEM vertical prerequisites",
    }
    write_new_json(output_path, receipt)
    print(json.dumps({"status": receipt["status"], "receipt": output_ref}, indent=2))
    return 0 if receipt["status"] == "pass_full_radar_header_readiness_only" else 20
