#!/usr/bin/env python3
"""Exact controls for M2 radar Esri-sequence recovery-005."""
...
CONTRACT_REF = "config/qa/m2-radar-esri-sequence-recovery-005-contract.json"
...
EXPECTED_AUTHORITY = {
    ...
    "review_bundle_ref": "reviews/m2-radar-esri-sequence-recovery-005/review-bundle.json",
    "review_bundle_sha256": "19af7c293e91bea1e5c46092263e67ce064781f10601f3139024457817b0bcd0",
    "official_method_audit_ref": "records/observations/m2-radar-gtc-despeckle-prerequisite-audit-001.json",
    "official_method_audit_sha256": "1237c8e8cc6d3905228dd7797782782b0262d634489c62d56d6850c213e7f4eb",
}
SOURCE_OUTPUT_NAMES = (
...
def validate_contract(contract: dict[str, Any], root: Path) -> list[str]:
    errors: list[str] = []
    if contract.get("contract_id") != "NEPAL-M2-RADAR-ESRI-SEQUENCE-RECOVERY-005":
    ...
    for stem in ("approval", "review_reconciliation", "activation", "proposal", "review_bundle", "official_method_audit"):
    ...
    if contract.get("receipt_namespace") != "RADAR-ESRI-SEQUENCE-RECOVERY-005":
    ...
    prior_ref = contract.get("prior_raster_function_contract_ref")
    if prior_ref != "config/qa/m2-radar-raster-function-call-shape-recovery-004-contract.json":
        errors.append("prior raster-function contract ref differs")
    else:
        try:
            if contract.get("prior_raster_function_contract_sha256") != base.sha256_file(_repo_file(root, prior_ref)):
                errors.append("prior raster-function contract bytes differ")
        except base.RadarRouteError:
            errors.append("prior raster-function contract missing")
    ...
    if call_shape.get("exact_sequence") != [
        ...
        errors.append("Raster function sequence differs")
    method = contract.get("scientific_method_amendment", {})
    if (
        method.get("historical_primary_despeckle") != "NONE"
        or method.get("recovery_005_primary_despeckle") != "REFINED_LEE"
        or method.get("position") != "after_ApplyRadiometricTerrainFlattening_before_ApplyGeometricTerrainCorrection_gamma"
        or method.get("polarization_bands") != "VV;VH"
        or method.get("filter_size_argument") != "omitted"
        or method.get("historical_contracts_immutable") is not True
        or method.get("recovery_specific_overlay") is not True
    ):
        errors.append("scientific method amendment differs")
...
def load_execution_plan(root: Path, contract_ref: str = CONTRACT_REF) -> dict[str, Any]:
    ...
    if errors:
        raise base.RadarRouteError("esri_sequence_recovery_005_contract_invalid", "; ".join(errors))
...
def stage_positions(stages: Iterable[str]) -> list[int]:
    ...
    for stage in stages:
        if stage not in positions:
            raise base.RadarRouteError("unknown_esri_sequence_recovery_005_stage", stage)
