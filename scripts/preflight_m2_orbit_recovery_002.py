#!/usr/bin/env python3
"""Run the final deterministic no-payload preflight for M2 orbit recovery-002."""
...
def main() -> int:
    ...
    if destination.exists() or staging_root.exists() or free_bytes < minimum_free_bytes:
        raise SystemExit("destination, staging, or storage preflight failed")
    payload = {
        "schema_version": "1.0",
        "receipt_id": "NEPAL-M2-ORBIT-RECOVERY-002-FINAL-PREFLIGHT-001",
