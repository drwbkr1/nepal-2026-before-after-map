#!/usr/bin/env python3
"""One distinct recovery-004 event-area radar QA process; no baseline or change."""
...
from m2_radar_event_pair_gtc_tiff_warp_recovery_004_processing import process_source
from m2_radar_event_pair_gtc_tiff_warp_recovery_004_core import MIN_FREE_BYTES
from pixel_qa_core import evaluate_aoi_coverage


PREFIX = "m2-radar-event-area-pair-gtc-tiff-warp-recovery-004"
...
CORE = ROOT / "scripts/m2_radar_event_pair_gtc_tiff_warp_recovery_004_core.py"
PROCESSING = ROOT / "scripts/m2_radar_event_pair_gtc_tiff_warp_recovery_004_processing.py"
DISPOSABLE_RECEIPT = ROOT / "records/readiness" / f"{PREFIX}-arcgis-disposable-005.json"
PAIRED_ROUTE_ID = "PAIR-S1-ASC-R085-IW"
REQUIRED_AOIS = ("AOI-SOURCE", "AOI-UPPER-CORRIDOR")
SUPERVISOR_MIN_FREE_BYTES = 60 * 1024**3
...
def verify_authority(*, verify_mosaic_bytes: bool = False) -> dict[str, Any]:
    ...
    if (sha256_file(PROPOSAL) != "69f69b8685f3f0daf9d00a361377085fccb6e954d4ba8d16e40241f2a6c8fd92"
            or sha256_file(BUNDLE) != "1378fb4057dfa47c36f83366c47936dae553e3c52dfee9a710a9c4757b94e93d"
            ...
            or packet.get("bindings", {}).get("public_ci_conclusion") != "success"):
        raise IntakeError("recovery_004_exact_authority_or_packet_gate_missing")
...
def event_plan(*, verify_dem_bytes: bool = False) -> dict[str, Any]:
    ...
    contract["attempt"] = {"attempt_id": "radar-event-area-pair-gtc-tiff-warp-recovery-004-real-001", "external_attempt_root": str(ATTEMPT_ROOT)}
    contract["attempt_local_receipts"] = True
    contract["receipt_namespace"] = "RADAR-EVENT-AREA-PAIR-GTC-TIFF-WARP-RECOVERY-004"
    ...
    if len(event_bounds) != 2:
        raise IntakeError("recovery_004_event_aoi_bounds_missing")
...
def projected_paths(plan: dict[str, Any], *, allow_reserved: bool = False) -> dict[str, Any]:
    ...
    for source in plan["sources"]:
        ...
        for name in SOURCE_OUTPUT_NAMES[:10] + (
                "gamma0_linear_native_bridge.tif", "gamma0_db_native_bridge.tif",
                "geometric_distortion_mask_native_bridge.tif",
                "gamma0_linear_epsg32645_10m.tif", "gamma0_db_epsg32645_10m.tif",
                "geometric_distortion_mask_epsg32645_10m.tif"):
...
def no_content_preflight(arcpy: Any) -> dict[str, Any]:
    ...
    if (gate.get("status") != "pass_public_ci_event_pair_radar_implementation"
            ...
            or gate.get("processing_sha256") != sha256_file(PROCESSING)
            or gate.get("core_sha256") != sha256_file(CORE)
            or gate.get("disposable_receipt_sha256") != sha256_file(DISPOSABLE_RECEIPT)
            or read_json(DISPOSABLE_RECEIPT).get("status") != "pass_generated_production_bridge_core_only"
            or gate.get("packet_gate_sha256") != sha256_file(PACKET_GATE)
...
def evaluate_single_source_aoi(arcpy: Any, plan: dict[str, Any], support: dict[str, Any],
    ...
    linear_path = source_root / "gamma0_linear_epsg32645_10m.tif"
    mask_path = source_root / "geometric_distortion_mask_epsg32645_10m.tif"
    qa_mask = source_root / "single_source_exclusion_mask.tif"
    linear_vv = make_band_layer(arcpy, linear_path, f"recovery004_{source_id.lower()}_vv", 1)
    try:
        ...
        arcpy.management.Delete(f"recovery004_{source_id.lower()}_vv")
...
def evaluate_pair_route(arcpy: Any, plan: dict[str, Any], support: dict[str, Any]) -> dict[str, Any]:
    ...
    before_linear = source_before / "gamma0_linear_epsg32645_10m.tif"
    after_linear = source_after / "gamma0_linear_epsg32645_10m.tif"
    before_mask_path = source_before / "geometric_distortion_mask_epsg32645_10m.tif"
    after_mask_path = source_after / "geometric_distortion_mask_epsg32645_10m.tif"
...
def run_one(*, reserved_by_supervisor: bool = False) -> dict[str, Any]:
    if not reserved_by_supervisor or os.environ.get("NEPAL_RECOVERY_004_WORKER") != "1":
        raise IntakeError("recovery_004_worker_requires_supervisor")
    ...
    if (not ATTEMPT_ROOT.is_dir() or (ATTEMPT_ROOT / "terminal.json").stat().st_size != 0
        ...
        raise IntakeError("recovery_004_supervisor_reservation_missing_or_used")
...
def _supervisor_resource_stop() -> str | None:
    if shutil.disk_usage(DATA_ROOT).free < SUPERVISOR_MIN_FREE_BYTES:
        return "supervisor_disk_free_below_60_gib"
    for source in ("2", "5"):
...
def run_supervised() -> dict[str, Any]:
    ...
    if (preflight.get("status") != "pass_final_no_content_two_source_radar_preflight"
        ...
        raise IntakeError("recovery_004_supervisor_preflight_or_attempt_collision")
    _reserve()
    command = [sys.executable, str(Path(__file__).resolve()), "--run-one"]
    environment = os.environ.copy()
    environment["NEPAL_RECOVERY_004_WORKER"] = "1"
