...
def inspect_one(
    *,
    ...
    inventory = select_required_members(manifest, contract)
    if inventory["status"] != "pass_inventory_only":
        return {
            "source_id": source_id,
...
def main() -> int:
    ...
    contract = load_path(ROOT / "config/qa/optical-input-readiness-contract.json")
