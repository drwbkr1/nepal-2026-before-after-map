#!/usr/bin/env python3
"""Append-only ArcGIS test of projected coverage and a single aligned Clip."""
...
ATTEMPT = ROOT / "scratch/map-route-feasibility-disposable-002"
...
PUBLIC_RECEIPT = ROOT / "records/readiness/m2-map-route-feasibility-001-arcgis-disposable-002.json"
...
def run() -> dict:
    ...
    try:
        ...
        for name, categorical, bands in (("continuous", False, 2), ("categorical", True, 1)):
            ...
            intermediate = ATTEMPT / f"{name}_intermediate_32645.tif"
            final = ATTEMPT / f"{name}_final_32645.tif"
    ...
    public = {"schema_version": "1.0", "record_id": "NEPAL-M2-MAP-ROUTE-FEASIBILITY-001-ARCGIS-DISPOSABLE-002",
