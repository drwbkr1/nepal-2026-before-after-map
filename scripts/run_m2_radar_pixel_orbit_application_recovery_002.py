#!/usr/bin/env python3
"""Run the one approved stage-evidenced radar recovery-002 attempt."""
...
from m2_radar_pixel_orbit_application_recovery_002_core import (
    ATTEMPT_ID,
    BASE_CONTRACT_REF,
    CONTRACT_REF,
...
RECOVERY_001_CORE_REF = "scripts/m2_radar_pixel_orbit_application_recovery_001_core.py"
...
def repository_bindings() -> dict[str, str]:
    return {
        ...
        "recovery_001_contract_sha256": repo_sha(RECOVERY_001_CONTRACT_REF),
        "recovery_001_core_sha256": repo_sha(RECOVERY_001_CORE_REF),
    }
...
def publication_gate() -> dict[str, Any]:
    ...
    if not path.is_file():
        raise base_core.RadarRouteError("recovery_002_implementation_publication_gate_missing")
    ...
    if (
        ...
        raise base_core.RadarRouteError("recovery_002_implementation_publication_gate_not_exact_pass")
    return gate
...
def gate_state_publication() -> dict[str, Any]:
    ...
    if not path.is_file():
        raise base_core.RadarRouteError("recovery_002_gate_state_publication_missing")
    ...
    if (
        ...
        raise base_core.RadarRouteError("recovery_002_gate_state_publication_not_exact_pass")
    return state
...
def build_final_preflight(checked_at_utc: str) -> dict[str, Any]:
    ...
    if target.exists() or (ROOT / TERMINAL_REF).exists():
        raise base_core.RadarRouteError("recovery_002_attempt_or_terminal_collision")
...
def _reserve_attempt(target: Path, executed_at_utc: str, bindings: dict[str, Any]) -> tuple[Path, Path]:
    if target.exists():
        raise base_core.RadarRouteError("recovery_002_attempt_collision")
    ...
    write_new_json(target / "started.json", {
        "schema_version": "1.0",
        "status": "started_single_stage_evidenced_recovery_002_attempt",
...
def execute_attempt_body(
    ...
    *,
    ...
    dem_builder: Callable[[Any, dict[str, Any], Path], Any] = base_runner.create_dem_mosaic,
    support_builder: Callable[[Any, dict[str, Any], Path], Any] = base_runner.create_analysis_support,
    source_processor: Callable[..., dict[str, Any]] = base_runner.process_source,
    route_processor: Callable[..., dict[str, Any]] = base_runner.process_route,
    ...
    terminal_status = "block_recovery_002_failure_no_retry"
    ...
    try:
        ...
        terminal_status = (
            ...
            else "block_recovery_002_failure_no_retry"
        )
        try:
