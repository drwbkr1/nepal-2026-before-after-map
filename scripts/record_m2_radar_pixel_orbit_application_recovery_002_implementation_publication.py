#!/usr/bin/env python3
"""Record successful public CI for the recovery-002 implementation."""
...
from m2_radar_pixel_orbit_application_recovery_002_core import write_new_json
from run_m2_radar_pixel_orbit_application_recovery_002 import repository_bindings
...
CHECKPOINT = "M2-RADAR-PIXEL-ORBIT-APPLICATION-RECOVERY-002-EXECUTION"
NEXT_ACTION = (
    "Publish and publicly validate the exact recovery-002 execution-gate state. Do not run the final no-content "
    "preflight, read project data or external custody, invoke production ArcPy, or start the fresh attempt until "
    "that gate-state commit passes public CI."
)
...
def main() -> int:
    ...
    if readiness.get("status") != "pass_recovery_002_implementation_public_ci_pending":
        raise SystemExit("implementation readiness differs")

    gate = {
        "schema_version": "1.0",
        "record_id": "NEPAL-M2-RADAR-PIXEL-ORBIT-APPLICATION-RECOVERY-002-IMPLEMENTATION-PUBLICATION-GATE",
        "recorded_at_utc": args.recorded_at_utc,
        "status": "pass_public_default_branch_ci_recovery_002_implementation_ready",
        ...
        "released_now": {
            ...
            "production_arcpy_invocation": False,
    ...
    reconciliation = {
        "schema_version": "1.0",
        "record_id": "NEPAL-M2-RADAR-PIXEL-ORBIT-APPLICATION-RECOVERY-002-IMPLEMENTATION-PUBLICATION-RECONCILIATION",
        "recorded_at_utc": args.recorded_at_utc,
        ...
        "implementation_commit_sha": args.implementation_commit_sha,
        "public_ci_run_id": args.public_ci_run_id,
        "released_now": {"gate_state_publication": True, "final_no_content_preflight": False, "fresh_attempt": False},
        "next_action": NEXT_ACTION,
    }
    ...
    implementation = units["M2-RADAR-PIXEL-ORBIT-APPLICATION-RECOVERY-002-IMPLEMENTATION"]
    ...
    implementation["exit_condition_delta"] = {
        "expected": [],
        "observed": ["exact recovery-002 implementation passed public default-branch CI"],
        "decision_value": "enables_dependency",
        "rationale": "Only execution gate-state publication is released now; final preflight and the fresh attempt remain blocked.",
    }
    ...
    evidence = {
        "record_id": "EVID-0185",
        "type": "m2_radar_pixel_orbit_application_recovery_002_implementation_publication_gate",
        "verified_at_utc": args.recorded_at_utc,
        "status": "pass_public_ci_gate_state_publication_pending",
        "claim": "The exact recovery-002 implementation commit passed public default-branch CI. Only gate-state publication is released; final preflight, project-data access, and the fresh attempt remain blocked.",
        "implementation_publication_gate_ref": GATE_REF,
        "implementation_publication_gate_sha256": sha256_file(ROOT / GATE_REF),
