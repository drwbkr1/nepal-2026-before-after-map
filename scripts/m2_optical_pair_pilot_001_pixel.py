#!/usr/bin/env python3
"""Run one new-pair QA-only pixel attempt with the frozen optical predicates."""
...
HEADER_RECEIPT = PILOT_ROOT / "header/header-001.json"
ATTEMPT_ROOT = PILOT_ROOT / "pixel-qa/pixel-qa-001"
PUBLIC_RECEIPT = ROOT / "records/readiness/optical-pixel/m2-optical-pair-pilot-001-pixel-qa.json"
...
def frozen_execution_contract(source_ids: list[str]) -> dict[str, Any]:
    ...
    frozen["attempt"] = {
        "attempt_id": "m2-optical-pair-pilot-001-pixel-qa-001",
        "maximum_real_invocations": 1,
        "automatic_retry_authorized": False,
        "external_attempt_root": str(ATTEMPT_ROOT),
        "public_receipt_ref": str(PUBLIC_RECEIPT.relative_to(ROOT)).replace("\\", "/"),
...
def run_qa_pixels(sources: list[dict[str, Any]], *, arcpy: Any) -> dict[str, Any]:
    ...
    try:
        ...
        failure = {
            "schema_version": "1.0", "receipt_id": "NEPAL-M2-OPTICAL-PAIR-PILOT-001-PIXEL-QA",
