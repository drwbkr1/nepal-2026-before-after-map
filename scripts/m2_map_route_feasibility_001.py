#!/usr/bin/env python3
"""One distinct e1/a3 map-route radar QA process; no baseline or change."""
...
from m2_radar_event_pair_dem_footprint_gate_001 import scan_dem_validity
...
from m2_map_route_feasibility_001_processing import process_source
from m2_map_route_feasibility_001_processing import require_materialization_space
from pixel_qa_core import evaluate_aoi_coverage
from m2_map_route_feasibility_001_core import TARGET, MAX_INTERMEDIATE_EXTENSION_M


PREFIX = "m2-map-route-feasibility-001"
ATTEMPT_ROOT = DATA_ROOT / "e1" / "a3"
IMPLEMENTATION_GATE = ROOT / "records/readiness" / f"{PREFIX}-radar-implementation-gate.json"
EXECUTION_GATE = ROOT / "records/readiness" / f"{PREFIX}-radar-execution-gate.json"
PREFLIGHT = ROOT / "records/readiness" / f"{PREFIX}-radar-final-preflight.json"
DEM_ASSESSMENT = ROOT / "records/readiness" / f"{PREFIX}-radar-processing-extent-dem-assessment.json"
DISPOSABLE_RECEIPT = ROOT / "records/readiness" / f"{PREFIX}-arcgis-disposable-004.json"
...
# This distinct runner may activate only after the exact public implementation,
# disposable ArcGIS, execution-CI, and final no-content preflight gates pass.
...
def verify_authority(*, verify_mosaic_bytes: bool = False) -> dict[str, Any]:
    ...
    if (sha256_file(PROPOSAL) != "6a0e568e7db0af2fbbd8d317a4324c4fda4c9784d069c9aaaa2697e3e12d436a"
            or sha256_file(BUNDLE) != "9419b4c0dc4272f14ec59521fa268d592368ccab6b8411c3a868f12e6739756e"
        ...
        raise IntakeError("map_route_exact_authority_or_packet_gate_missing")
...
def event_plan(*, verify_dem_bytes: bool = False) -> dict[str, Any]:
    ...
    contract["attempt"] = {"attempt_id": "map-route-feasibility-001-radar-real-001", "external_attempt_root": str(ATTEMPT_ROOT)}
    contract["attempt_local_receipts"] = True
    contract["receipt_namespace"] = "MAP-ROUTE-FEASIBILITY-001-RADAR"
    ...
    if len(event_bounds) != 2:
        raise IntakeError("map_route_event_aoi_bounds_missing")
...
def projected_paths(plan: dict[str, Any], *, allow_reserved: bool = False) -> dict[str, Any]:
    ...
    for source in plan["sources"]:
        ...
        paths.extend(root / name for name in (
            "gamma0_linear_epsg32645_10m_intermediate.crf",
            "gamma0_db_epsg32645_10m_intermediate.crf",
            "geometric_distortion_mask_epsg32645_10m_intermediate.crf",
        ))
...
def no_content_preflight(arcpy: Any) -> dict[str, Any]:
    if PREFLIGHT.exists() or DEM_ASSESSMENT.exists():
        raise IntakeError("event_pair_radar_preflight_collision")
    gate = read_json(IMPLEMENTATION_GATE)
    execution = read_json(EXECUTION_GATE)
    disposable = read_json(DISPOSABLE_RECEIPT)
    if (gate.get("status") != "pass_public_ci_map_route_implementation"
            or gate.get("runner_sha256") != sha256_file(Path(__file__))
            or gate.get("processor_sha256") != sha256_file(ROOT / "scripts/m2_map_route_feasibility_001_processing.py")
            or gate.get("core_sha256") != sha256_file(ROOT / "scripts/m2_map_route_feasibility_001_core.py")
            or gate.get("disposable_receipt_sha256") != sha256_file(DISPOSABLE_RECEIPT)
            or gate.get("disposable_terminal_sha256") != disposable.get("attempt_terminal_sha256")
            or execution.get("status") != "pass_public_ci_map_route_execution_gate"
            or execution.get("implementation_gate_sha256") != sha256_file(IMPLEMENTATION_GATE)
            or disposable.get("status") != "pass_disposable_grid_and_sampled_pixels_only"
    ...
    return {
        "schema_version": "1.0",
        "record_id": "NEPAL-M2-MAP-ROUTE-FEASIBILITY-001-RADAR-FINAL-PREFLIGHT",
        "checked_at_utc": now_utc(),
        "status": "pass_final_no_content_two_source_radar_preflight",
        "implementation_gate_sha256": sha256_file(IMPLEMENTATION_GATE),
        "execution_gate_sha256": sha256_file(EXECUTION_GATE),
        "disposable_receipt_sha256": sha256_file(DISPOSABLE_RECEIPT),
...
def run_dem_assessment() -> dict[str, Any]:
    """Exhaustively check DEM validity over the maximum admitted processing grid."""
    ...
    if (preflight.get("status") != "pass_final_no_content_two_source_radar_preflight"
            ...
            or DEM_ASSESSMENT.exists() or ATTEMPT_ROOT.exists()):
        raise IntakeError("map_route_dem_assessment_gate_or_identity_collision")
    with DEM_ASSESSMENT.open("xb") as stream:
        stream.flush()
        os.fsync(stream.fileno())
    try:
        plan = event_plan(verify_dem_bytes=True)
        identities = verify_input_identities(plan)
        ...
        if source_crs.ImportFromEPSG(32645) != 0 or destination_crs.ImportFromEPSG(4326) != 0:
            raise IntakeError("map_route_dem_assessment_crs_unavailable")
        for crs in (source_crs, destination_crs):
            crs.SetAxisMappingStrategy(osr.OAMS_TRADITIONAL_GIS_ORDER)
        transform = osr.CoordinateTransformation(source_crs, destination_crs)
        pad = MAX_INTERMEDIATE_EXTENSION_M
        west, south, east, north = (TARGET[0] - pad, TARGET[1] - pad,
                                    TARGET[2] + pad, TARGET[3] + pad)
        perimeter = []
        for start, end in (((west, south), (east, south)), ((east, south), (east, north)),
            ...
            for step in range(16):
                ...
                perimeter.append((lon, lat))
        ring = ogr.Geometry(ogr.wkbLinearRing)
        for lon, lat in perimeter + [perimeter[0]]:
            ring.AddPoint_2D(lon, lat)
        polygon = ogr.Geometry(ogr.wkbPolygon)
        polygon.AddGeometry(ring)
        if not polygon.IsValid():
            raise IntakeError("map_route_dem_assessment_polygon_invalid")
        polygon = polygon.Buffer(0.001)
        if polygon is None or polygon.IsEmpty() or not polygon.IsValid():
            raise IntakeError("map_route_dem_assessment_buffer_invalid")
        ...
        if dataset is None or dataset.RasterXSize != 14400 or dataset.RasterYSize != 10800:
            raise IntakeError("map_route_dem_assessment_mosaic_unreadable")
        scan = scan_dem_validity(dataset, [polygon, polygon], gdal=gdal, ogr=ogr, np=np)
        if scan["invalid_dem_cells"] != 0:
            raise IntakeError("map_route_processing_extent_has_invalid_dem")
        terminal = {"schema_version": "1.0", "status": "pass_actual_sources_and_full_processing_extent_valid_dem",
                    "checked_at_utc": now_utc(), "source_order": list(IDS),
                    "source_inventory_sha256_by_id": {key: value["inventory_sha256"] for key, value in identities["sources"].items()},
                    "mosaic_sha256": identities["mosaic"], "processing_extent_maximum_bounds_32645_m": [west, south, east, north],
                    "conservative_wgs84_buffer_degrees": 0.001,
                    "dem_scan": scan, "radar_geoprocessing_started": False,
                    "baseline_or_change_analysis_authorized": False}
    except BaseException as exc:
        terminal = {"schema_version": "1.0", "status": "block_dem_assessment_no_radar_attempt",
                    "checked_at_utc": now_utc(), **safe_error(exc, (DATA_ROOT, ATTEMPT_ROOT)),
                    "radar_geoprocessing_started": False, "baseline_or_change_analysis_authorized": False}
    _write_reserved(DEM_ASSESSMENT, terminal)
    return terminal
...
def evaluate_single_source_aoi(arcpy: Any, plan: dict[str, Any], support: dict[str, Any],
    ...
    linear_vv = make_band_layer(arcpy, linear_path, f"maproute001_{source_id.lower()}_vv", 1)
    try:
        ...
        arcpy.management.Delete(f"maproute001_{source_id.lower()}_vv")
...
def run_one(*, reserved_by_supervisor: bool = False) -> dict[str, Any]:
    if not reserved_by_supervisor or os.environ.get("NEPAL_MAP_ROUTE_001_WORKER") != "1":
        raise IntakeError("map_route_worker_requires_supervisor")
    preflight = read_json(PREFLIGHT)
    dem_assessment = read_json(DEM_ASSESSMENT)
    if (preflight.get("status") != "pass_final_no_content_two_source_radar_preflight"
            or preflight.get("implementation_gate_sha256") != sha256_file(IMPLEMENTATION_GATE)
            or preflight.get("execution_gate_sha256") != sha256_file(EXECUTION_GATE)
            or preflight.get("footprint_gate_sha256") != sha256_file(FOOTPRINT_GATE)
            or dem_assessment.get("status") != "pass_actual_sources_and_full_processing_extent_valid_dem"
            or dem_assessment.get("mosaic_sha256") != sha256_file(MOSAIC)):
    ...
    if (not ATTEMPT_ROOT.is_dir() or (ATTEMPT_ROOT / "terminal.json").stat().st_size != 0
        ...
        raise IntakeError("map_route_supervisor_reservation_missing_or_used")
    ...
    def marker(*, source_id: str, tool: str, phase: str) -> None:
        if phase in {"started", "native_presave_metadata_passed"}:
            require_materialization_space(ATTEMPT_ROOT)
    ...
    try:
        ...
        pair_pass = passed_sources and route is not None and route["disposition"] == "pass_qa_only"
        terminal = {
            "schema_version": "1.0",
            "record_id": "NEPAL-M2-MAP-ROUTE-FEASIBILITY-001-RADAR-TERMINAL",
            "status": ("pass_pair_qa_only" if pair_pass else
                       "terminal_pair_route_qa_not_passed" if route is not None else
        ...
        terminal = {
            "schema_version": "1.0", "record_id": "NEPAL-M2-MAP-ROUTE-FEASIBILITY-001-RADAR-TERMINAL",
...
def run_supervised() -> dict[str, Any]:
    ...
    if (preflight.get("status") != "pass_final_no_content_two_source_radar_preflight"
        ...
        raise IntakeError("map_route_supervisor_preflight_or_attempt_collision")
    _reserve()
    command = [sys.executable, str(Path(__file__).resolve()), "--run-one"]
    environment = os.environ.copy()
    environment["NEPAL_MAP_ROUTE_001_WORKER"] = "1"
...
def main() -> int:
    ...
    group.add_argument("--assess-dem", action="store_true")
    ...
    try:
        if args.preflight:
            ...
            return 0
        if args.assess_dem:
            value = run_dem_assessment()
            print(json.dumps({"status": value["status"]}))
            return 0 if value["status"] == "pass_actual_sources_and_full_processing_extent_valid_dem" else 20
        ...
        return 0 if value["status"] == "pass_pair_qa_only" else 20
