#!/usr/bin/env python3
"""Record successful public CI for the exact delayed-import probe implementation."""
...
from m2_radar_delayed_import_probe_001_core import canonical_bytes, sha256_file, write_new_json
from run_m2_radar_delayed_import_probe_001 import repository_bindings


ROOT = Path(__file__).resolve().parents[1]
GATE_REF = "records/readiness/m2-radar-delayed-import-probe-001-implementation-publication-gate.json"
RECONCILIATION_REF = "records/readiness/m2-radar-delayed-import-probe-001-implementation-publication-reconciliation.json"
READINESS_REF = "records/readiness/m2-radar-delayed-import-probe-001-implementation-readiness.json"
...
NEXT_ACTION = (
    "Publish and publicly validate the exact implementation-gate state. Do not run the final no-content preflight, create the disposable "
    "corpus, or import ArcPy until that gate-state commit passes public CI."
)
...
def main() -> int:
    ...
    if len(args.implementation_commit_sha) != 40:
        raise SystemExit("implementation commit must be a full SHA")
    ...
    gate = {
        "schema_version": "1.0",
        "record_id": "NEPAL-M2-RADAR-DELAYED-IMPORT-PROBE-001-IMPLEMENTATION-PUBLICATION-GATE",
        "recorded_at_utc": args.recorded_at_utc,
        "status": "pass_public_default_branch_ci_probe_implementation_ready",
        ...
        "released_now": {
            ...
            "final_no_content_preflight_after_gate_state_publication": True,
            "live_probe": False,
    ...
    reconciliation = {
        "schema_version": "1.0",
        "record_id": "NEPAL-M2-RADAR-DELAYED-IMPORT-PROBE-001-IMPLEMENTATION-PUBLICATION-RECONCILIATION",
        "recorded_at_utc": args.recorded_at_utc,
        "status": "pass_public_implementation_gate_gate_state_publication_pending",
    ...
    implementation = units["M2-RADAR-DELAYED-IMPORT-PROBE-001-IMPLEMENTATION"]
    execution = units["M2-RADAR-DELAYED-IMPORT-PROBE-001-EXECUTION"]
    ...
    implementation["exit_condition_delta"] = {
        "expected": [],
        "observed": ["exact implementation passed public default-branch CI"],
        "decision_value": "enables_dependency",
        "rationale": "Only publication of the exact gate state is released now; the final preflight remains blocked.",
    }
    execution["status"] = "in_progress"
    execution["gates"].update({"public_ci": "success", "gate_state_publication": "pending"})
    ...
    evidence = {
        "record_id": "EVID-0169",
        "type": "m2_radar_delayed_import_probe_001_implementation_publication_gate",
        "verified_at_utc": args.recorded_at_utc,
        "status": "pass_public_ci_gate_state_publication_pending",
        "claim": "Exact delayed-import probe implementation commit passed public default-branch CI. Only publication of this gate state is released; the final preflight and live probe remain blocked.",
        "implementation_publication_gate_ref": GATE_REF,
        "implementation_publication_gate_sha256": sha256_file(ROOT / GATE_REF),
        ...
        "assertions": {
            ...
            "public_test_count": args.public_test_count,
            "public_intentional_skip_count": args.public_skip_count,
            "gate_state_publication_pending": True,
            "final_no_content_preflight_performed": False,
            "probe_process_started": False,
            "disposable_corpus_created": False,
            "arcpy_imported": False,
    ...
    ledger = ROOT / "records/evidence-ledger.jsonl"
