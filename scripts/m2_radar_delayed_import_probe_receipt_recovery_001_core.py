#!/usr/bin/env python3
"""Portable controls for delayed-import probe receipt recovery-001."""
...
EXPECTED_AGGREGATE_SHA256 = "dd56f8b28a1ed1c6e2b4b1d7d8f5db4fd86dab80a910fe79c8d018d58942430b"
...
def local_attempt_root() -> Path:
    ...
    return (
        ...
        / "diagnostics"
        / "radar-delayed-import-probe-receipt-recovery-001"
        / ATTEMPT_ID
    )
...
def validate_scan(summary: dict[str, Any], *, enforce_expected_digest: bool = True) -> None:
    ...
    if [item.get("name") for item in summary.get("files", [])] != list(CORPUS_NAMES):
        raise ProbeError("corpus_hash_order_mismatch")
    if enforce_expected_digest and summary.get("aggregate_sha256") != EXPECTED_AGGREGATE_SHA256:
        raise ProbeError("corpus_hash_aggregate_mismatch")
