#!/usr/bin/env python3
"""Distinct map-route e1/a3 source processor; closed attempts stay unchanged."""
...
from m2_radar_event_pair_native_gtc_recovery_003_core import (
    ...
    native_gtc_environment,
)
from m2_map_route_feasibility_001_core import MapRouteStop, TARGET, project_then_clip


MIN_FREE_BYTES = 40 * 1024**3


def require_materialization_space(attempt_root: Path) -> None:
    # Every admitted single output is capped at 10 GiB; 40 GiB exceeds twice
    # that maximum declared size and is the approved minimum free-space floor.
    if shutil.disk_usage(attempt_root).free < MIN_FREE_BYTES:
        raise MapRouteStop("materialization_free_space_below_40_gib")


def compare_retained_windows(arcpy: Any, np: Any, intermediate: Path,
                             final: Path, *, categorical: bool) -> dict[str, Any]:
    """Verify representative target cells survive Clip unchanged; AOI QA remains separate."""
    locations = ((TARGET[0] + 1000, TARGET[1] + 1000),
                 (TARGET[0] + 40000, TARGET[1] + 20000),
                 (TARGET[2] - 3000, TARGET[3] - 3000))
    observations = []
    for x, y in locations:
        options = {"lower_left_corner": arcpy.Point(x, y), "ncols": 128,
                   "nrows": 128, "nodata_to_value": 255 if categorical else -9999.0}
        middle = arcpy.RasterToNumPyArray(str(intermediate), **options)
        clipped = arcpy.RasterToNumPyArray(str(final), **options)
        same = bool(middle.shape == clipped.shape and np.array_equal(middle, clipped))
        if not same:
            raise MapRouteStop("sampled_retained_pixels_changed")
        observations.append({"lower_left_m": [x, y], "shape": list(clipped.shape),
                             "retained_values_identical": True})
    return {"window_size_cells": [128, 128], "windows": observations,
...
def process_source(
    *,
    ...
    try:
        ...
        gamma_intermediate = source_root / "gamma0_linear_epsg32645_10m_intermediate.crf"
        db_intermediate = source_root / "gamma0_db_epsg32645_10m_intermediate.crf"
        mask_intermediate = source_root / "geometric_distortion_mask_epsg32645_10m_intermediate.crf"
        ...
        projected_grids = {
            "gamma": project_then_clip(arcpy, gamma_gtc_raw, gamma_intermediate, gamma_final,
                                        support["snap"], categorical=False, expected_bands=2,
                                        before_materialization=lambda _: require_materialization_space(attempt_root)),
            "db": project_then_clip(arcpy, db_gtc_raw, db_intermediate, db_final,
                                     support["snap"], categorical=False, expected_bands=2,
                                     before_materialization=lambda _: require_materialization_space(attempt_root)),
            "mask": project_then_clip(arcpy, mask_gtc_raw, mask_intermediate, mask_final,
                                       support["snap"], categorical=True, expected_bands=1,
                                       before_materialization=lambda _: require_materialization_space(attempt_root)),
        }
        import numpy as np  # type: ignore  # installed ArcGIS runtime only

        for name, middle, final, categorical in (
            ("gamma", gamma_intermediate, gamma_final, False),
            ("db", db_intermediate, db_final, False),
            ("mask", mask_intermediate, mask_final, True),
        ):
            projected_grids[name]["sampled_pixel_equivalence"] = compare_retained_windows(
                arcpy, np, middle, final, categorical=categorical)
