...
import m2_radar_esri_sequence_recovery_005_core as core  # noqa: E402
import m2_radar_esri_sequence_processing_005 as short_processing  # noqa: E402
import run_m2_radar_esri_sequence_recovery_005 as runner  # noqa: E402
...
class M2RadarEsriSequenceRecovery005Tests(unittest.TestCase):
    ...
    def test_contract_exact_and_one_attempt_boundary(self) -> None:
        ...
        self.assertEqual(contract["scientific_method_amendment"]["recovery_005_primary_despeckle"], "REFINED_LEE")
        self.assertEqual(contract["scientific_method_amendment"]["filter_size_argument"], "omitted")
        self.assertTrue(contract["scientific_method_amendment"]["historical_contracts_immutable"])
...
class M2RadarEsriSequencePipeline005Tests(unittest.TestCase):
    ...
    def test_exact_raster_function_call_shapes_and_one_save_each(self) -> None:
        ...
        self.assertEqual([len(args) for _, args in tool_calls], [2, 3, 8, 3, 4, 2, 4])
        ...
        self.assertEqual(tool_calls[3][1][1:], ("VV;VH", "REFINED_LEE"))
        self.assertEqual(tool_calls[4][1][0], str(outputs["gamma_despeckled"]))
        self.assertEqual(len([item for item in calls if item[0] == "save"]), 7)
        self.assertTrue(all(markers[index]["phase"] == ("started" if index % 2 == 0 else "completed") for index in range(14)))
        self.assertEqual([markers[index]["tool"] for index in range(0, 14, 2)], [name for name, _ in tool_calls])
