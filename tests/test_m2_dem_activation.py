...
class M2DemActivationTests(unittest.TestCase):
    ...
    def test_milestone_and_profile_preserve_parallel_dem_route(self) -> None:
        ...
        expected_checkpoint = "M2-RADAR-PIXEL-ORBIT-APPLICATION-RECOVERY-002-REVIEW-PUBLICATION" if all_verified else ("M2-DEM-GEOTIFF-VERIFICATION" if all_promoted else "M2-DEM-ACQUISITION")
        ...
        self.assertEqual(
            ...
            ["contracts/milestone-002-radar-pixel-orbit-application-recovery-002-proposal.json"],
        )
        self.assertEqual(
        ...
        orbit_recovery_002_review = next(
        ...
        orbit_osv_precision_publication = next(
            (
                unit
                for unit in self.milestone["units"]
                if unit["id"] == "M2-ORBIT-OSV-PRECISION-AMENDMENT-001-REVIEW-PUBLICATION"
            ),
            None,
        )
        ...
        orbit_offline_recovery_implementation = next(
            (
                unit
                for unit in self.milestone["units"]
                if unit["id"] == "M2-ORBIT-OFFLINE-VERIFICATION-RECOVERY-001-IMPLEMENTATION"
            ),
            None,
        )
        orbit_offline_recovery = next(
            (
                unit
                for unit in self.milestone["units"]
                if unit["id"] == "M2-ORBIT-OFFLINE-VERIFICATION-RECOVERY-001"
            ),
            None,
        )
        expected_primary_checkpoint = (
            "M2-RADAR-PIXEL-ORBIT-APPLICATION-RECOVERY-002-REVIEW-PUBLICATION"
            ...
            else "M2-ORBIT-OFFLINE-VERIFICATION-RECOVERY-001-IMPLEMENTATION"
            if orbit_offline_recovery_implementation is not None
            and orbit_offline_recovery_implementation.get("status") == "in_progress"
            else "M2-ORBIT-OFFLINE-VERIFICATION-RECOVERY-001-REVIEW"
        ...
        self.assertEqual(len(self.profile["parallel_checkpoints"]), 1)
